...
func (m *CPUMiner) submitBlock(block *lddlutil.Block) bool {
	...
	minrLog.Infof("Block submitted via CPU miner accepted (hash %s, "+
		...
		lddlutil.Amount(coinbaseTxGenerated))
	return true
}
...
out:
	for {
		...
		// Hacks to make lddl work with Legenddigital PoC (simnet only)
		...
		if m.solveBlock(template.Block, ticker, quit) {
			block := lddlutil.NewBlock(template.Block)
			m.submitBlock(block)
