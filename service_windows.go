...
const (
	// svcName is the name of lddld service.
	svcName = "lddldsvc"
	...
	svcDisplayName = "Lddld Service"

	// svcDesc is the description of the service.
	svcDesc = "Downloads and stays synchronized with the Legenddigital block " +
...
// logServiceStartOfDay logs information about lddld when the main server has
...
// lddldService houses the main service handler which handles all service
// updates and launching lddldMain.
type lddldService struct{}
...
// long-running lddldMain (which is the real meat of lddld), handles service
// change requests, and notifies the service control manager of changes.
func (s *lddldService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start lddldMain in a separate goroutine so the service can start
...
// installService attempts to install the lddld service.  Typically this should
...
// removeService attempts to uninstall the lddld service.  Typically this should
...
// startService attempts to start the lddld service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &lddldService{})
	if err != nil {
