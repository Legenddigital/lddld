...
const (
	...
	unusableFlags = lddljson.UFWebsocketOnly | lddljson.UFNotification
)

var (
	lddldHomeDir           = lddlutil.AppDataDir("lddld", false)
	lddlctlHomeDir         = lddlutil.AppDataDir("lddlctl", false)
	lddlwalletHomeDir      = lddlutil.AppDataDir("lddlwallet", false)
	defaultConfigFile      = filepath.Join(lddlctlHomeDir, "lddlctl.conf")
	defaultRPCServer       = "localhost"
	defaultWalletRPCServer = "localhost"
	defaultRPCCertFile     = filepath.Join(lddldHomeDir, "rpc.cert")
	defaultWalletCertFile  = filepath.Join(lddlwalletHomeDir, "rpc.cert")
)
...
func listCommands() {
	...
	cmdMethods := lddljson.RegisteredCmdMethods()
	categorized := make([][]string, numCategories)
	for _, method := range cmdMethods {
		flags, err := lddljson.MethodUsageFlags(method)
		if err != nil {
		...
		usage, err := lddljson.MethodUsageText(method)
		if err != nil {
			// This should never happen since the method was just
			// returned from the package, but be safe.
			continue
		...
		if flags&lddljson.UFWalletOnly != 0 {
...
// config defines the configuration options for lddlctl.
//
...
// For this it tries to read the lddld config file at its default path, and extract
// the RPC user and password from it.
func createDefaultConfigFile(destinationPath string) error {
	// Nothing to do when there is no existing lddld conf file at the default
	// path to extract the details from.
	lddldConfigPath := filepath.Join(lddldHomeDir, "lddld.conf")
	if !fileExists(lddldConfigPath) {
		return nil
	}

	// Read lddld.conf from its default path
	lddldConfigFile, err := os.Open(lddldConfigPath)
	if err != nil {
		return err
	}
	defer lddldConfigFile.Close()
	content, err := ioutil.ReadAll(lddldConfigFile)
	if err != nil {
		return err
	}
