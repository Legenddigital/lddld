...
func main() {
	...
	method := args[0]
	usageFlags, err := lddljson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := lddljson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// lddljson.Error as it reallistcally will always be since the
		// NewCmd function is only supposed to return errors of that
		// type.
		if jerr, ok := err.(lddljson.Error); ok {
		...
		// The error is not a lddljson.Error and this really should not
	...
	marshalledJSON, err := lddljson.MarshalCmd("1.0", 1, cmd)
	if err != nil {
