...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *lddlutil.Block) {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *lddlutil.Block) {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *lddlutil.Tx, isNew bool) {
...
func (f *wsClientFilter) addAddress(a lddlutil.Address) {
	switch a := a.(type) {
...
func (f *wsClientFilter) addAddressStr(s string) {
	a, err := lddlutil.DecodeAddress(s)
...
func (f *wsClientFilter) existsAddress(a lddlutil.Address) bool {
	switch a := a.(type) {
	case *lddlutil.AddressPubKeyHash:
...
// Notification types
type notificationBlockConnected lddlutil.Block
type notificationBlockDisconnected lddlutil.Block
...
func (m *wsNotificationManager) subscribedClients(tx *lddlutil.Tx, clients map[chan struct{}]*wsClient) map[chan struct{}]struct{} {
...
func (m *wsNotificationManager) notifyBlockConnected(clients map[chan struct{}]*wsClient, block *lddlutil.Block) {
	...
	ntfn := lddljson.BlockConnectedNtfn{
	...
	for quitChan, client := range clients {
		...
		marshalledJSON, err := lddljson.MarshalCmd("1.0", nil, &ntfn)
		if err != nil {
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *lddlutil.Block) {
	...
	ntfn := lddljson.BlockDisconnectedNtfn{
		Header: hex.EncodeToString(headerBytes),
	}
	marshalledJSON, err := lddljson.MarshalCmd("1.0", nil, &ntfn)
	if err != nil {
...
func (m *wsNotificationManager) notifyReorganization(clients map[chan struct{}]*wsClient, rd *blockchain.ReorganizationNtfnsData) {
	...
	// Notify interested websocket clients about the disconnected block.
	ntfn := lddljson.NewReorganizationNtfn(rd.OldHash.String(),
		int32(rd.OldHeight),
		rd.NewHash.String(),
		int32(rd.NewHeight))
	marshalledJSON, err := lddljson.MarshalCmd("1.0", nil, ntfn)
...
func (*wsNotificationManager) notifyWinningTickets(
	...
	ntfn := lddljson.NewWinningTicketsNtfn(wtnd.BlockHash.String(),
...
func (*wsNotificationManager) notifySpentAndMissedTickets(clients map[chan struct{}]*wsClient, tnd *blockchain.TicketNotificationsData) {
	...
	// Notify interested websocket clients about the connected block.
	ntfn := lddljson.NewSpentAndMissedTicketsNtfn(tnd.Hash.String(),
...
func (*wsNotificationManager) notifyNewTickets(clients map[chan struct{}]*wsClient, tnd *blockchain.TicketNotificationsData) {
	...
	// Notify interested websocket clients about the connected block.
	ntfn := lddljson.NewNewTicketsNtfn(tnd.Hash.String(), int32(tnd.Height),
...
func (*wsNotificationManager) notifyStakeDifficulty(clients map[chan struct{}]*wsClient, sdnd *StakeDifficultyNtfnData) {
	// Notify interested websocket clients about the connected block.
	ntfn := lddljson.NewStakeDifficultyNtfn(sdnd.BlockHash.String(),
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *lddlutil.Tx) {
	...
	ntfn := lddljson.NewTxAcceptedNtfn(txHashStr,
		lddlutil.Amount(amount).ToCoin())
	marshalledJSON, err := lddljson.MarshalCmd("1.0", nil, ntfn)
	if err != nil {
	...
	var verboseNtfn *lddljson.TxAcceptedVerboseNtfn
	...
	for _, wsc := range clients {
		if wsc.verboseTxUpdates {
			...
			verboseNtfn = lddljson.NewTxAcceptedVerboseNtfn(*rawTx)
			marshalledJSONVerbose, err = lddljson.MarshalCmd("1.0", nil,
				verboseNtfn)
			if err != nil {
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *lddlutil.Tx,
	...
	if len(clientsToNotify) != 0 {
		n := lddljson.NewRelevantTxAcceptedNtfn(txHexString(msgTx))
		marshalled, err := lddljson.MarshalCmd("1.0", nil, n)
		if err != nil {
	...
	for {
		...
		if !batchedRequest {
			...
			if err != nil {
				...
				jsonErr := &lddljson.RPCError{
					Code:    lddljson.ErrRPCParse.Code,
			...
			if req.Method == "" || req.Params == nil {
				jsonErr := &lddljson.RPCError{
					Code:    lddljson.ErrRPCInvalidRequest.Code,
			...
			switch authCmd, ok := cmd.cmd.(*lddljson.AuthenticateCmd); {
			...
			if !c.isAdmin {
				if _, ok := rpcLimited[req.Method]; !ok {
					jsonErr := &lddljson.RPCError{
						Code:    lddljson.ErrRPCInvalidParams.Code,
		...
		if batchedRequest {
			...
			if err != nil {
				...
				reply, err = lddljson.MarshalResponse("2.0", nil, nil, jsonErr)
				if err != nil {
...
func handleWebsocketHelp(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*lddljson.HelpCmd)
	if !ok {
		return nil, lddljson.ErrRPCInternal
	}
...
func handleLoadTxFilter(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*lddljson.LoadTxFilterCmd)
...
func handleSession(wsc *wsClient, icmd interface{}) (interface{}, error) {
	return &lddljson.SessionResult{SessionID: wsc.sessionID}, nil
}
...
func handleNotifyNewTransactions(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*lddljson.NotifyNewTransactionsCmd)
	if !ok {
		return nil, lddljson.ErrRPCInternal
	}
...
// string slice.
func rescanBlock(filter *wsClientFilter, block *lddlutil.Block) []string {
...
func handleRescan(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*lddljson.RescanCmd)
	if !ok {
		return nil, lddljson.ErrRPCInternal
	}
	...
	if filter == nil {
		return nil, &lddljson.RPCError{
			Code:    lddljson.ErrRPCMisc,
			Message: "Transaction filter must be loaded before rescanning",
		}
	}

	blockHashes, err := lddljson.DecodeConcatenatedHashes(cmd.BlockHashes)
	if err != nil {
		return nil, err
	}

	discoveredData := make([]lddljson.RescannedBlock, 0, len(blockHashes))
	...
	for i := range blockHashes {
		...
		if err != nil {
			return nil, &lddljson.RPCError{
				Code:    lddljson.ErrRPCBlockNotFound,
		...
		if len(transactions) != 0 {
			discoveredData = append(discoveredData, lddljson.RescannedBlock{
	...
	return &lddljson.RescanResult{DiscoveredData: discoveredData}, nil
}
