...
import (
	...
	"github.com/Legenddigital/lddld/blockchain/internal/dbnamespace"
...
func serializeSizeForMinimalOutputs(tx *lddlutil.Tx) int {
...
func putTxToMinimalOutputs(target []byte, tx *lddlutil.Tx) int {
...
// already there.
func dbMaybeStoreBlock(dbTx database.Tx, block *lddlutil.Block) error {
...
// The amount is never encoded into spent transaction outputs in Legenddigital
...
func decodeSpentTxOut(serialized []byte, stxo *spentTxOut, amount int64, height uint32, index uint32) (int, error) {
	...
	// since in Legenddigital we only need pkScript at most due to fraud proofs
...
// txouts.
func dbFetchSpendJournalEntry(dbTx database.Tx, block *lddlutil.Block, parent *lddlutil.Block) ([]spentTxOut, error) {
...
func (b *BlockChain) createChainState() error {
	...
	genesisBlock := lddlutil.NewBlock(b.chainParams.GenesisBlock)
...
// block for the provided node, deserialize it, and return a lddlutil.Block.
func dbFetchBlockByNode(dbTx database.Tx, node *blockNode) (*lddlutil.Block, error) {
	...
	block, err := lddlutil.NewBlockFromBytes(blockBytes)
	if err != nil {
		return nil, err
	}
