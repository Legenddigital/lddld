...
// block.
func ticketsVotedInBlock(bl *lddlutil.Block) []chainhash.Hash {
...
// block.
func ticketsRevokedInBlock(bl *lddlutil.Block) []chainhash.Hash {
	var tickets []chainhash.Hash
	for _, stx := range bl.MsgBlock().STransactions {
...
func upgradeToVersion2(db database.DB, chainParams *chaincfg.Params, dbInfo *databaseInfo) error {
	...
	dbFetchBlockByHeight := func(dbTx database.Tx, height int64) (*lddlutil.Block, error) {
