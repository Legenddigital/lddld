...
type Indexer interface {
	...
	ConnectBlock(dbTx database.Tx, block, parent *lddlutil.Block, view *blockchain.UtxoViewpoint) error

	// DisconnectBlock is invoked when the index manager is notified that a
	// block has been disconnected from the main chain.
	DisconnectBlock(dbTx database.Tx, block, parent *lddlutil.Block, view *blockchain.UtxoViewpoint) error
}
...
// be considered valid.
func approvesParent(block *lddlutil.Block) bool {
	return lddlutil.IsFlagSet16(block.MsgBlock().Header.VoteBits,
		lddlutil.BlockValid)
}
