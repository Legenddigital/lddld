...
import (
	...
	"github.com/Legenddigital/lddld/blockchain/internal/progresslog"
...
func dbIndexConnectBlock(dbTx database.Tx, indexer Indexer, block, parent *lddlutil.Block, view *blockchain.UtxoViewpoint) error {
...
func dbIndexDisconnectBlock(dbTx database.Tx, indexer Indexer, block, parent *lddlutil.Block, view *blockchain.UtxoViewpoint) error {
...
// block for the provided hash, deserialize it, and return a lddlutil.Block.
func dbFetchBlockByHash(dbTx database.Tx, hash *chainhash.Hash) (*lddlutil.Block, error) {
	blockBytes, err := dbTx.FetchBlock(hash)
	if err != nil {
		return nil, err
	}
	return lddlutil.NewBlockFromBytes(blockBytes)
}
...
func (m *Manager) Init(chain *blockchain.BlockChain, interrupt <-chan struct{}) error {
	...
	var cachedBlock *lddlutil.Block
	...
	var cachedParent *lddlutil.Block
	for height := lowestHeight + 1; height <= bestHeight; height++ {
		if interruptRequested(interrupt) {
			return errInterruptRequested
		}

		var block, parent *lddlutil.Block
		err = m.db.Update(func(dbTx database.Tx) error {
...
func makeUtxoView(dbTx database.Tx, block, parent *lddlutil.Block, interrupt <-chan struct{}) (*blockchain.UtxoViewpoint, error) {
	...
	for txIdx, tx := range parentRegularTxs {
		...
		for _, txIn := range tx.MsgTx().TxIn {
			...
			view.AddTxOuts(lddlutil.NewTx(originTx),
	...
	for _, tx := range block.STransactions() {
		...
		for i, txIn := range msgTx.TxIn {
			...
			view.AddTxOuts(lddlutil.NewTx(originTx), int64(wire.NullBlockHeight),
				wire.NullBlockIndex)
		}
...
//
// This is part of the blockchain.IndexManager interface.
func (m *Manager) ConnectBlock(dbTx database.Tx, block, parent *lddlutil.Block, view *blockchain.UtxoViewpoint) error {
...
//
// This is part of the blockchain.IndexManager interface.
func (m *Manager) DisconnectBlock(dbTx database.Tx, block, parent *lddlutil.Block, view *blockchain.UtxoViewpoint) error {
	// Call each of the currently active optional indexes with the block
