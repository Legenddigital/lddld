...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := lddlutil.Hash160(redeemScript)
...
// subsidy.
func additionalCoinbasePoW(amount lddlutil.Amount) func(*wire.MsgBlock) {
...
func additionalCoinbaseDev(amount lddlutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
		// Increase the first proof-of-work coinbase subsidy by the
		// provided amount.
...
func additionalSpendFee(fee lddlutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
func Generate(includeLargeReorg bool) (tests [][]TestInstance, err error) {
	...
	g.NextBlock("bbadtaxscript", outs[5], ticketOuts[5],
		func(b *wire.MsgBlock) {
			...
			p2shTaxAddr := addrs[0].(*lddlutil.AddressScriptHash)
			p2pkhTaxAddr, err := lddlutil.NewAddressPubKeyHash(
	...
	g.NextBlock("bv15", outs[9], ticketOuts[9], func(b *wire.MsgBlock) {
		ticketFee := lddlutil.Amount(2)
		ticketPrice := lddlutil.Amount(g.CalcNextReqStakeDifficulty(g.Tip()))
		ticketPrice--
	...
	// vector is not possible in Legenddigital since the block size is included in
	...
	// Legenddigital enforces the coinbase includes the block height to which it
	...
	g.NextBlock("bcb9", outs[19], ticketOuts[19], func(b *wire.MsgBlock) {
		b.Transactions[1].TxOut[0].Value = lddlutil.MaxAmount + 1
	})
	rejected(blockchain.ErrBadTxOutValue)
	...
	g.NextBlock("bcb10", outs[19], ticketOuts[19], func(b *wire.MsgBlock) {
		b.Transactions[1].TxOut[0].Value = lddlutil.MaxAmount
