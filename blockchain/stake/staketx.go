...
func AddrFromSStxPkScrCommitment(pkScript []byte,
	params *chaincfg.Params) (lddlutil.Address, error) {
	...
	if isP2SH {
		addr, err = lddlutil.NewAddressScriptHashFromHash(hashBytes, params)
	} else {
		addr, err = lddlutil.NewAddressPubKeyHash(hashBytes, params,
...
// ticket commitment pkScript.
func AmountFromSStxPkScrCommitment(pkScript []byte) (lddlutil.Amount, error) {
	if len(pkScript) < SStxPKHMinOutSize {
	...
	return lddlutil.Amount(binary.LittleEndian.Uint64(amtEncoded)), nil
}
