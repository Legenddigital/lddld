...
//
// This function is safe for concurrent access.
func (b *BlockChain) TicketsWithAddress(address lddlutil.Address) ([]chainhash.Hash, error) {
	b.chainLock.RLock()
	sn := b.bestChain.Tip().stakeNode
	b.chainLock.RUnlock()
...
func (b *BlockChain) TicketPoolValue() (lddlutil.Amount, error) {
	b.chainLock.RLock()
	sn := b.bestChain.Tip().stakeNode
	b.chainLock.RUnlock()
	...
	return lddlutil.Amount(amt), nil
}
