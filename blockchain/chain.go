...
// BlockChain provides functions for working with the Legenddigital block chain.
...
type BlockChain struct {
	...
	mainchainBlockCache     map[chainhash.Hash]*lddlutil.Block
...
// StakeVersions is a condensed form of a lddlutil.Block that is used to prevent
...
// exceeded.
func (b *BlockChain) addOrphanBlock(block *lddlutil.Block) {
...
func (b *BlockChain) fetchMainChainBlockByNode(node *blockNode) (*lddlutil.Block, error) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) fetchBlockByNode(node *blockNode) (*lddlutil.Block, error) {
...
func (b *BlockChain) pushMainChainBlockCache(block *lddlutil.Block) {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBlock(node *blockNode, block, parent *lddlutil.Block, view *UtxoViewpoint, stxos []spentTxOut) error {
	...
	blockAndParent := []*lddlutil.Block{block, parent}
...
func (b *BlockChain) dropMainChainBlockCache(block *lddlutil.Block) {
	curHash := block.Hash()
	b.mainchainBlockCacheLock.Lock()
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) disconnectBlock(node *blockNode, block, parent *lddlutil.Block, view *UtxoViewpoint) error {
...
func countSpentOutputs(block *lddlutil.Block, parent *lddlutil.Block) int {
...
// adding the block.
func countNumberOfTransactions(block, parent *lddlutil.Block) uint64 {
	var numTxns uint64
	if headerApprovesParent(&block.MsgBlock().Header) {
...
func (b *BlockChain) reorganizeChain(detachNodes, attachNodes *list.List) error {
	...
	detachBlocks := make([]*lddlutil.Block, 0, detachNodes.Len())
	detachSpentTxOuts := make([][]spentTxOut, 0, detachNodes.Len())
	attachBlocks := make([]*lddlutil.Block, 0, attachNodes.Len())
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBestChain(node *blockNode, block, parent *lddlutil.Block, flags BehaviorFlags) (int64, error) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) BlockByHash(hash *chainhash.Hash) (*lddlutil.Block, error) {
	node := b.index.LookupNode(hash)
	if node == nil {
		return nil, fmt.Errorf("block %s is not known", hash)
	}
...
//
// This function is safe for concurrent access.
func (b *BlockChain) BlockByHeight(height int64) (*lddlutil.Block, error) {
...
type IndexManager interface {
	...
	// main chain.
	ConnectBlock(database.Tx, *lddlutil.Block, *lddlutil.Block, *UtxoViewpoint) error

	// DisconnectBlock is invoked when a block has been disconnected from
	// the main chain.
	DisconnectBlock(database.Tx, *lddlutil.Block, *lddlutil.Block, *UtxoViewpoint) error
}
...
func New(config *Config) (*BlockChain, error) {
	...
	b := BlockChain{
		...
		mainchainBlockCache:           make(map[chainhash.Hash]*lddlutil.Block),
