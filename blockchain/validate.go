...
func IsCoinBase(tx *lddlutil.Tx) bool {
...
// level util transaction as opposed to a raw wire transaction.
func IsExpired(tx *lddlutil.Tx, blockHeight int64) bool {
...
func CheckTransactionSanity(tx *wire.MsgTx, params *chaincfg.Params) error {
	...
	// known as an atom.  One Legenddigital is a quantity of atoms as defined by
	...
	for _, txOut := range tx.TxOut {
		...
		// is detected and reported.  This is impossible for Legenddigital,
		...
		if totalAtom > lddlutil.MaxAmount {
			str := fmt.Sprintf("total value of all transaction "+
...
func checkProofOfStake(block *lddlutil.Block, posLimit int64) error {
...
func checkBlockSanity(block *lddlutil.Block, timeSource MedianTimeSource, flags BehaviorFlags, chainParams *chaincfg.Params) error {
	msgBlock := block.MsgBlock()
...
// sane before continuing with block processing.  These checks are context
// free.
func CheckBlockSanity(block *lddlutil.Block, timeSource MedianTimeSource, chainParams *chaincfg.Params) error {
...
// context free.
func CheckWorklessBlockSanity(block *lddlutil.Block, timeSource MedianTimeSource, chainParams *chaincfg.Params) error {
...
func (b *BlockChain) checkBlockContext(block *lddlutil.Block, prevNode *blockNode, flags BehaviorFlags) error {
	// The genesis block is valid by definition.
	if prevNode == nil {
		return nil
	}
...
//
// Legenddigital: Check the stake transactions to make sure they don't have this txid
// too.
func (b *BlockChain) checkDupTxs(txSet []*lddlutil.Tx, view *UtxoViewpoint) error {
...
// owner of the Legenddigital and therefore allowed to spend them.  As it checks the
// inputs, it also calculates the total fees for the transaction and returns
// that value.
//
// NOTE: The transaction MUST have already been sanity checked with the
// CheckTransactionSanity function prior to calling this function.
func CheckTransactionInputs(subsidyCache *SubsidyCache, tx *lddlutil.Tx, txHeight int64, utxoView *UtxoViewpoint, checkFraudProof bool, chainParams *chaincfg.Params) (int64, error) {
	msgTx := tx.MsgTx()
	...
	// -------------------------------------------------------------------
	// Legenddigital stake transaction testing.
	// -------------------------------------------------------------------
	...
	// -------------------------------------------------------------------
	// Legenddigital general transaction testing (and a few stake exceptions).
	// -------------------------------------------------------------------
	for idx, txIn := range msgTx.TxIn {
		...
		// Legenddigital is a quantity of atoms as defined by the AtomPerCoin
		// constant.
		...
		if originTxAtom > lddlutil.MaxAmount {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v",
				originTxAtom, lddlutil.MaxAmount)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		...
		if totalAtomIn < lastAtomIn ||
			totalAtomIn > lddlutil.MaxAmount {
			str := fmt.Sprintf("total value of all transaction "+
...
// txscript.
func CountSigOps(tx *lddlutil.Tx, isCoinBaseTx bool, isSSGen bool) int {
	msgTx := tx.MsgTx()
...
func CountP2SHSigOps(tx *lddlutil.Tx, isCoinBaseTx bool, isStakeBaseTx bool, utxoView *UtxoViewpoint) (int, error) {
...
func checkNumSigOps(tx *lddlutil.Tx, utxoView *UtxoViewpoint, index int, txTree bool, cumulativeSigOps int) (int, error) {
	msgTx := tx.MsgTx()
	isSSGen := stake.IsSSGen(msgTx)
...
func checkStakeBaseAmounts(subsidyCache *SubsidyCache, height int64, params *chaincfg.Params, txs []*lddlutil.Tx, utxoView *UtxoViewpoint) error {
	for _, tx := range txs {
		msgTx := tx.MsgTx()
...
func getStakeBaseAmounts(txs []*lddlutil.Tx, utxoView *UtxoViewpoint) (int64, error) {
	totalInputs := int64(0)
	totalOutputs := int64(0)
	for _, tx := range txs {
		msgTx := tx.MsgTx()
...
func getStakeTreeFees(subsidyCache *SubsidyCache, height int64, params *chaincfg.Params, txs []*lddlutil.Tx, utxoView *UtxoViewpoint) (lddlutil.Amount, error) {
	totalInputs := int64(0)
	totalOutputs := int64(0)
	for _, tx := range txs {
		msgTx := tx.MsgTx()
	...
	return lddlutil.Amount(totalInputs - totalOutputs), nil
}
...
func (b *BlockChain) checkTransactionsAndConnect(subsidyCache *SubsidyCache, inputFees lddlutil.Amount, node *blockNode, txs []*lddlutil.Tx, utxoView *UtxoViewpoint, stxos *[]spentTxOut, txTree bool) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkConnectBlock(node *blockNode, block, parent *lddlutil.Block, utxoView *UtxoViewpoint, stxos *[]spentTxOut) error {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) CheckConnectBlockTemplate(block *lddlutil.Block) error {
	b.chainLock.Lock()
	...
	var nextBlockToDetach *lddlutil.Block
	...
	var prevAttachBlock *lddlutil.Block
