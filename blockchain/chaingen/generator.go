...
func (s *SpendableOut) Amount() lddlutil.Amount {
	return s.amount
}
...
func makeSpendableOutForTxInternal(tx *wire.MsgTx, blockHeight, txIndex, txOutIndex uint32, tree int8) SpendableOut {
	return SpendableOut{
		...
		blockIndex:  txIndex,
		amount:      lddlutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
type Generator struct {
	...
	p2shOpTrueAddr   lddlutil.Address
...
func MakeGenerator(params *chaincfg.Params) (Generator, error) {
	...
	p2shOpTrueAddr, err := lddlutil.NewAddressScriptHash(opTrueScript, params)
	if err != nil {
...
// a single OP_TRUE.
func (g *Generator) P2shOpTrueAddr() lddlutil.Address {
...
func (g *Generator) calcFullSubsidy(blockHeight uint32) lddlutil.Amount {
	...
	return lddlutil.Amount(subsidy)
}
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcPoWSubsidy(fullSubsidy lddlutil.Amount, blockHeight uint32, numVotes uint16) lddlutil.Amount {
	powProportion := lddlutil.Amount(g.params.WorkRewardProportion)
	totalProportions := lddlutil.Amount(g.params.TotalSubsidyProportions())
	...
	ticketsPerBlock := lddlutil.Amount(g.params.TicketsPerBlock)
	return (powSubsidy * lddlutil.Amount(numVotes)) / ticketsPerBlock
}
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcPoSSubsidy(heightVotedOn uint32) lddlutil.Amount {
	if int64(heightVotedOn+1) < g.params.StakeValidationHeight {
		return 0
	}

	fullSubsidy := g.calcFullSubsidy(heightVotedOn)
	posProportion := lddlutil.Amount(g.params.StakeRewardProportion)
	totalProportions := lddlutil.Amount(g.params.TotalSubsidyProportions())
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcDevSubsidy(fullSubsidy lddlutil.Amount, blockHeight uint32, numVotes uint16) lddlutil.Amount {
	devProportion := lddlutil.Amount(g.params.BlockTaxProportion)
	totalProportions := lddlutil.Amount(g.params.TotalSubsidyProportions())
	...
	// Reduce the subsidy according to the number of votes.
	ticketsPerBlock := lddlutil.Amount(g.params.TicketsPerBlock)
	return (devSubsidy * lddlutil.Amount(numVotes)) / ticketsPerBlock
}
...
func (g *Generator) addCoinbaseTxOutputs(tx *wire.MsgTx, blockHeight uint32, devSubsidy, powSubsidy lddlutil.Amount) {
...
func PurchaseCommitmentScript(addr lddlutil.Address, amount, voteFeeLimit, revocationFeeLimit lddlutil.Amount) []byte {
...
func (g *Generator) CreateTicketPurchaseTx(spend *SpendableOut, ticketPrice, fee lddlutil.Amount) *wire.MsgTx {
...
func (g *Generator) CreateVoteTx(voteBlock *wire.MsgBlock, ticketTx *wire.MsgTx, ticketBlockHeight, ticketBlockIndex uint32) *wire.MsgTx {
	...
	voteSubsidy := posSubsidy / lddlutil.Amount(g.params.TicketsPerBlock)
	ticketPrice := lddlutil.Amount(ticketTx.TxOut[0].Value)
...
// children nodes.  A diagram depicting how this works for Legenddigital transactions
...
// will be nil.
func buildMerkleTreeStore(transactions []*lddlutil.Tx) []*chainhash.Hash {
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	utilTxns := make([]*lddlutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, lddlutil.NewTx(tx))
	}
...
//
// This compact form is only used in Legenddigital to encode unsigned 256-bit numbers
...
func (g *Generator) CreateSpendTx(spend *SpendableOut, fee lddlutil.Amount) *wire.MsgTx {
...
// opTrueRedeemScript.
func (g *Generator) CreateSpendTxForTx(tx *wire.MsgTx, blockHeight, txIndex uint32, fee lddlutil.Amount) *wire.MsgTx {
...
func (g *Generator) NextBlock(blockName string, spend *SpendableOut, ticketSpends []SpendableOut, mungers ...func(*wire.MsgBlock)) *wire.MsgBlock {
	...
	ticketPrice := lddlutil.Amount(g.CalcNextRequiredStakeDifficulty())
	...
	if nextHeight > uint32(g.params.CoinbaseMaturity) {
		...
		// outputs.
		if ticketSpends != nil {
			const ticketFee = lddlutil.Amount(2)
	...
	var stakeTreeFees lddlutil.Amount
	for txIdx, tx := range stakeTxns {
		...
		var inputSum, outputSum lddlutil.Amount
		for _, txIn := range tx.TxIn {
			inputSum += lddlutil.Amount(txIn.ValueIn)
		}
		for _, txOut := range tx.TxOut {
			outputSum += lddlutil.Amount(txOut.Value)
		}
...
func (g *Generator) CreatePremineBlock(blockName string, additionalAmount lddlutil.Amount, mungers ...func(*wire.MsgBlock)) *wire.MsgBlock {
	...
	var totalSubsidy lddlutil.Amount
	for _, payout := range g.params.BlockOneLedger {
		payoutAddr, err := lddlutil.DecodeAddress(payout.Address)
		if err != nil {
			panic(err)
		}
		...
		totalSubsidy += lddlutil.Amount(payout.Amount)
	}
