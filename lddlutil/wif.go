...
func NewWIF(privKey chainec.PrivateKey, net *chaincfg.Params, ecType lddlec.SignatureType) (*WIF,
	error) {
	if net == nil {
...
func DecodeWIF(wif string) (*WIF, error) {
	...
	var ecType lddlec.SignatureType
	switch lddlec.SignatureType(decoded[2]) {
	...
	case lddlec.STEd25519:
		...
		ecType = lddlec.STEd25519
	case lddlec.STSchnorrSecp256k1:
		privKeyBytes := decoded[3 : 3+secp256k1.PrivKeyBytesLen]
		privKey, _ = secp256k1.PrivKeyFromScalar(privKeyBytes)
		ecType = lddlec.STSchnorrSecp256k1
	}
...
func (w *WIF) DSA() lddlec.SignatureType {
	return w.ecType
}
