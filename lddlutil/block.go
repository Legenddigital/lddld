...
// Tx returns a wrapped transaction (lddlutil.Tx) for the transaction at the
...
// STx returns a wrapped transaction (lddlutil.Tx) for the stake transaction at
...
// Transactions returns a slice of wrapped transactions (lddlutil.Tx) for all
...
// instead provides easy access to wrapped versions (lddlutil.Tx) of them.
...
// STransactions returns a slice of wrapped stake transactions (lddlutil.Tx) for all
// stake transactions in the Block.  This is nearly equivalent to accessing the raw
// transactions (lddlwire.MsgTx) in the underlying wire.MsgBlock, however it
