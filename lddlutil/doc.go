...
A Block defines a Legenddigital block that provides easier and more efficient
...
A Tx defines a Legenddigital transaction that provides more efficient manipulation of
...
The Address interface provides an abstraction for a Legenddigital address.  While the
most common type is a pay-to-pubkey-hash, Legenddigital already supports others and
...
To decode/encode an address:
	...
	addr, err := lddlutil.DecodeAddress(addrString, defaultNet)
	if err != nil {
		fmt.Println(err)
		return
	}
