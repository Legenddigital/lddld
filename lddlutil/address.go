...
import (
	...
	"github.com/Legenddigital/base58"
	...
	"github.com/Legenddigital/lddld/lddlec/edwards"
	"github.com/Legenddigital/lddld/lddlec/secp256k1"
	"github.com/Legenddigital/lddld/lddlec/secp256k1/schnorr"
)
...
func encodePKAddress(serializedPK []byte, netID [2]byte, algo lddlec.SignatureType) string {
	pubKeyBytes := []byte{0x00}

	switch algo {
	case lddlec.STEcdsaSecp256k1:
		pubKeyBytes[0] = byte(lddlec.STEcdsaSecp256k1)
	case lddlec.STEd25519:
		pubKeyBytes[0] = byte(lddlec.STEd25519)
	case lddlec.STSchnorrSecp256k1:
		pubKeyBytes[0] = byte(lddlec.STSchnorrSecp256k1)
	}

	// Pubkeys are encoded as [0] = type/ybit, [1:33] = serialized pubkey
	compressed := serializedPK
	if algo == lddlec.STEcdsaSecp256k1 || algo == lddlec.STSchnorrSecp256k1 {
...
type Address interface {
	...
	DSA(*chaincfg.Params) lddlec.SignatureType
...
func NewAddressPubKey(decoded []byte, net *chaincfg.Params) (Address, error) {
	if len(decoded) == 33 {
		...
		switch lddlec.SignatureType(suite) {
		case lddlec.STEcdsaSecp256k1:
...
func DecodeAddress(addr string) (Address, error) {
	...
	case net.PubKeyHashAddrID:
		return NewAddressPubKeyHash(decoded, net, lddlec.STEcdsaSecp256k1)

	case net.PKHEdwardsAddrID:
		return NewAddressPubKeyHash(decoded, net, lddlec.STEd25519)

	case net.PKHSchnorrAddrID:
		return NewAddressPubKeyHash(decoded, net, lddlec.STSchnorrSecp256k1)
...
func NewAddressPubKeyHash(pkHash []byte, net *chaincfg.Params,
	algo lddlec.SignatureType) (*AddressPubKeyHash, error) {
	var addrID [2]byte
	switch algo {
	case lddlec.STEcdsaSecp256k1:
...
// hash.
func (a *AddressPubKeyHash) DSA(net *chaincfg.Params) lddlec.SignatureType {
	switch a.netID {
	case net.PubKeyHashAddrID:
		return lddlec.STEcdsaSecp256k1
	case net.PKHEdwardsAddrID:
		return lddlec.STEd25519
	case net.PKHSchnorrAddrID:
		return lddlec.STSchnorrSecp256k1
	}
	return -1
}
...
func (a *AddressScriptHash) DSA(net *chaincfg.Params) lddlec.SignatureType {
	return -1
}
...
func NewAddressSecpPubKey(serializedPubKey []byte,
	...
	// from lddlec, but do it here to avoid API churn.  We already know the
...
// differs with the format.  At the time of this writing, most Legenddigital addresses
...
// address.
func (a *AddressSecpPubKey) DSA(net *chaincfg.Params) lddlec.SignatureType {
...
// DSA returns the underlying digital signature algorithm for the
// address.
func (a *AddressEdwardsPubKey) DSA(net *chaincfg.Params) lddlec.SignatureType {
	return lddlec.STEd25519
}
...
// DSA returns the underlying digital signature algorithm for the
// address.
func (a *AddressSecSchnorrPubKey) DSA(net *chaincfg.Params) lddlec.SignatureType {
	return lddlec.STSchnorrSecp256k1
}
