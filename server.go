...
import (
	...
	"github.com/Legenddigital/lddld/peer"
...
var (
	...
	// ourselves to other Legenddigital peers.
	userAgentName = "lddld"
...
// broadcastMsg provides the ability to house a Legenddigital message to be broadcast
...
// server provides a Legenddigital server for handling communications to and from
// Legenddigital peers.
type server struct {
...
func (sp *serverPeer) OnTx(p *peer.Peer, msg *wire.MsgTx) {
	...
	// Convert the raw MsgTx to a lddlutil.Tx which provides some convenience
	// methods and things such as hash caching.
	tx := lddlutil.NewTx(msg)
...
func (sp *serverPeer) OnBlock(p *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	// Convert the raw MsgBlock to a lddlutil.Block which provides some
	// convenience methods and things such as hash caching.
	block := lddlutil.NewBlockFromBlockAndBytes(msg, buf)
...
func (s *server) AnnounceNewTransactions(newTxs []*lddlutil.Tx) {
...
func (s *server) peerHandler() {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, defaultRequiredServices, lddldLookup, func(addrs []*wire.NetAddress) {
...
// AddedNodeInfo returns an array of lddljson.GetAddedNodeInfoResult structures
...
// newServer returns a new lddld server configured to listen on addr for the
// Legenddigital network type specified by chainParams.  Use start to begin accepting
...
func newServer(listenAddrs []string, db database.DB, chainParams *chaincfg.Params, interrupt <-chan struct{}) (*server, error) {
	...
	amgr := addrmgr.New(cfg.DataDir, lddldLookup)
...
func addrStringToNetAddr(addr string) (net.Addr, error) {
	...
	// The lddldLookup function will transparently handle performing the
	// lookup over Tor if necessary.
	ips, err := lddldLookup(host)
	if err != nil {
		return nil, err
	}
	if len(ips) == 0 {
