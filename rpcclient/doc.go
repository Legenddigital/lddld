...
/*
Package rpcclient implements a websocket-enabled Legenddigital JSON-RPC client.

Overview

This client provides a robust and easy to use client for interfacing
with a Legenddigital RPC server that uses a mostly btcd/bitcoin core
style Legenddigital JSON-RPC API.  This client has been tested with lddld
(https://github.com/Legenddigital/lddld) and lddlwallet
(https://github.com/Legenddigital/lddlwallet).

In addition to the compatible standard HTTP POST JSON-RPC API, lddld and
lddlwallet provide a websocket interface that is more efficient than the standard
...
lddld or lddlwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by lddld and
lddlwallet only uses a single connection that remains open and allows
...
All notifications provided by lddld require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by lddld (and
lddlwallet passthrough).  Therefore if you call one of these commands against
...
Also, it is important to realize that lddld intentionally separates the wallet
functionality into a separate process named lddlwallet.  This means if you are
connected to the lddld RPC server directly, only the RPCs which are related to
chain services will be available.  Depending on your application, you might only
need chain-related RPCs.  In contrast, lddlwallet provides pass through treatment
...
detected by type asserting the error in a *lddljson.RPCError.  For example, to
detect if a command is unimplemented by the remote RPC server:

  amount, err := client.GetBalance("")
  if err != nil {
  	if jerr, ok := err.(*lddljson.RPCError); ok {
  		switch jerr.Code {
  		case lddljson.ErrRPCUnimplemented:
...
The following full-blown client examples are in the examples directory:
 ...
 - lddldwebsockets
   ...
   block count
 - lddlwalletwebsockets
   Connects to a lddlwallet RPC server using TLS-secured websockets, registers
