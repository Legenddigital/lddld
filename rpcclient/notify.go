...
type NotificationHandlers struct {
	...
	// OnLddldConnected is invoked when a wallet connects or disconnects from
	// lddld.
	//
	// This will only be available when client is connected to a wallet
	// server such as lddlwallet.
	OnLddldConnected func(connected bool)

	// OnAccountBalance is invoked with account balance updates.
	//
	// This will only be available when speaking to a wallet server
	// such as lddlwallet.
	OnAccountBalance func(account string, balance lddlutil.Amount, confirmed bool)
	...
	// This will only be available when client is connected to a wallet
	// server such as lddlwallet.
	OnTicketsPurchased func(TxHash *chainhash.Hash, amount lddlutil.Amount)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	switch ntfn.Method {
	// OnBlockConnected
	case lddljson.BlockConnectedNtfnMethod:
	...
	// OnBlockDisconnected
	case lddljson.BlockDisconnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case lddljson.RelevantTxAcceptedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case lddljson.ReorganizationNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnWinningTickets
	case lddljson.WinningTicketsNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case lddljson.SpentAndMissedTicketsNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnNewTickets
	case lddljson.NewTicketsNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnStakeDifficulty
	case lddljson.StakeDifficultyNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTxAccepted
	case lddljson.TxAcceptedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTxAcceptedVerbose
	case lddljson.TxAcceptedVerboseNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
		...
		// OnLddldConnected
	case lddljson.LddldConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
		if c.ntfnHandlers.OnLddldConnected == nil {
			return
		}

		connected, err := parseLddldConnectedNtfnParams(ntfn.Params)
		if err != nil {
			log.Warnf("Received invalid lddld connected "+
				"notification: %v", err)
			return
		}

		c.ntfnHandlers.OnLddldConnected(connected)

	// OnAccountBalance
	case lddljson.AccountBalanceNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTicketPurchased:
	case lddljson.TicketPurchasedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnVotesCreated:
	case lddljson.VoteCreatedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case lddljson.RevocationCreatedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnWalletLockState
	case lddljson.WalletLockStateNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
...
func parseTxAcceptedNtfnParams(params []json.RawMessage) (*chainhash.Hash,
	...
	// Bounds check amount.
	amt, err := lddlutil.NewAmount(famt)
	if err != nil {
		return nil, 0, err
	}
...
func parseTxAcceptedVerboseNtfnParams(params []json.RawMessage) (*lddljson.TxRawResult,
	error) {
	...
	var rawTx lddljson.TxRawResult
...
// parseLddldConnectedNtfnParams parses out the connection status of lddld
// and lddlwallet from the parameters of a lddldconnected notification.
func parseLddldConnectedNtfnParams(params []json.RawMessage) (bool, error) {
	if len(params) != 1 {
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance lddlutil.Amount, confirmed bool, err error) {
	...
	// Bounds check amount.
	bal, err := lddlutil.NewAmount(fbal)
	if err != nil {
		return "", 0, false, err
	}
...
func parseTicketPurchasedNtfnParams(params []json.RawMessage) (txHash *chainhash.Hash,
	amount lddlutil.Amount, err error) {
	...
	return thHash, lddlutil.Amount(amt), nil
}
...
//
// NOTE: This is a lddld extension and requires a websocket connection.
func (c *Client) NotifyBlocksAsync() FutureNotifyBlocksResult {
	...
	cmd := lddljson.NewNotifyBlocksCmd()
...
func (c *Client) NotifyWinningTicketsAsync() FutureNotifyWinningTicketsResult {
	...
	cmd := lddljson.NewNotifyWinningTicketsCmd()
...
func (c *Client) NotifySpentAndMissedTicketsAsync() FutureNotifySpentAndMissedTicketsResult {
	...
	cmd := lddljson.NewNotifySpentAndMissedTicketsCmd()
...
func (c *Client) NotifyNewTicketsAsync() FutureNotifyNewTicketsResult {
	...
	cmd := lddljson.NewNotifyNewTicketsCmd()
...
func (c *Client) NotifyStakeDifficultyAsync() FutureNotifyStakeDifficultyResult {
	...
	cmd := lddljson.NewNotifyStakeDifficultyCmd()
...
func (c *Client) NotifyNewTransactionsAsync(verbose bool) FutureNotifyNewTransactionsResult {
	...
	cmd := lddljson.NewNotifyNewTransactionsCmd(&verbose)
	return c.sendCmd(cmd)
}
...
//
// NOTE: This is a lddld extension and requires a websocket connection.
func (c *Client) LoadTxFilterAsync(reload bool, addresses []lddlutil.Address,
	...
	outPointObjects := make([]lddljson.OutPoint, len(outPoints))
	for i := range outPoints {
		outPointObjects[i] = lddljson.OutPoint{
	...
	cmd := lddljson.NewLoadTxFilterCmd(reload, addrStrs, outPointObjects)
	return c.sendCmd(cmd)
}
...
//
// NOTE: This is a lddld extension and requires a websocket connection.
func (c *Client) LoadTxFilter(reload bool, addresses []lddlutil.Address, outPoints []wire.OutPoint) error {
