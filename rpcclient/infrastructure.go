...
// Client represents a Legenddigital RPC client which allows easy access to the
// various RPC methods available on a Legenddigital RPC server.  Each of the wrapper
...
func (c *Client) trackRegisteredNtfns(cmd interface{}) {
	...
	case *lddljson.NotifyWinningTicketsCmd:
		c.ntfnState.notifyWinningTickets = true

	case *lddljson.NotifySpentAndMissedTicketsCmd:
		c.ntfnState.notifySpentAndMissedTickets = true

	case *lddljson.NotifyNewTicketsCmd:
		c.ntfnState.notifyNewTickets = true

	case *lddljson.NotifyStakeDifficultyCmd:
		c.ntfnState.notifyStakeDifficulty = true

	case *lddljson.NotifyBlocksCmd:
		c.ntfnState.notifyBlocks = true

	case *lddljson.NotifyNewTransactionsCmd:
...
type (
	...
	rawResponse struct {
		...
		Error  *lddljson.RPCError `json:"error"`
	}
)
...
// returning an unmarshaled lddljson.RPCError (or an unmarshaling error) if so.
...
func (c *Client) sendCmd(cmd interface{}) chan *response {
	...
	method, err := lddljson.CmdMethod(cmd)
	if err != nil {
	...
	// Marshal the command.
	id := c.NextID()
	marshalledJSON, err := lddljson.MarshalCmd("1.0", id, cmd)
	if err != nil {
		return newFutureError(err)
	}
