...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt lddlutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(lddlutil.Amount(5 * lddlutil.AtomsPerCoin))
	...
	txid = genSpend(lddlutil.Amount(5000 * lddlutil.AtomsPerCoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testJoinMempools(r *Harness, t *testing.T) {
	...
	pooledHashes, err := r.Node.GetRawMempool(lddljson.GRMAll)
	if err != nil {
	...
	go func() {
		for {
			poolHashes, err := r.Node.GetRawMempool(lddljson.GRMAll)
			if err != nil {
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := lddlutil.Amount(5 * 300 * lddlutil.AtomsPerCoin)
	...
	// chain.
	expectedBalance = lddlutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := lddlutil.Amount(50 * lddlutil.AtomsPerCoin)
...
func TestHarness(t *testing.T) {
	...
	expectedBalance := lddlutil.Amount(numMatureOutputs * 300 * lddlutil.AtomsPerCoin)
