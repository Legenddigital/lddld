...
// Harness fully encapsulates an active lddld process to provide a unified
// platform for creating rpc driven integration tests involving lddld. The
// active lddld node will typically be run in simnet mode in order to allow for
// easy generation of test blockchains.  The active lddld process is fully
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	...
	// wallet.
	filterAddrs := []lddlutil.Address{h.wallet.coinbaseAddr}
	if err := h.Node.LoadTxFilter(true, filterAddrs, nil); err != nil {
		return err
	}

	// Ensure lddld properly dispatches our registered call-back for each new
...
// connectRPCClient attempts to establish an RPC connection to the created lddld
...
func (h *Harness) NewAddress() (lddlutil.Address, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) ConfirmedBalance() lddlutil.Amount {
...
//
// This function is safe for concurrent access.
func (h *Harness) SendOutputs(targetOutputs []*wire.TxOut, feeRate lddlutil.Amount) (*chainhash.Hash, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) CreateTransaction(targetOutputs []*wire.TxOut, feeRate lddlutil.Amount) (*wire.MsgTx, error) {
