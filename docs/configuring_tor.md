...
lddld provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, lddld supports a hybrid,
...
While it is easier to only run as a client, it is more beneficial to the Legenddigital
...
Configuring lddld as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the lddld command
line or in the lddld configuration file.  Typically the Tor proxy address will be
...
By default, lddld assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since lddld will act as the server.
...
```bash
$ ./lddld --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
lddld provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the lddld command line or in the lddld
...
```bash
$ ./lddld --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
lddld provides support for Tor stream isolation by using the `--torisolation`
...
```bash
$ ./lddld --proxy=127.0.0.1:9050 --torisolation
```
