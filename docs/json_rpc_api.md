...
lddld provides a [JSON-RPC](http://json-rpc.org/wiki/specification) API that is
fully compatible with the original bitcoind/bitcoin-qt.  There are a few key
differences between lddld and bitcoind as far as how RPCs are serviced:
* Unlike bitcoind that has the wallet and chain intermingled in the same process
  which leads to several issues, lddld intentionally splits the wallet and chain
  ...
  talking directly to lddld, only chain-related RPCs are available.  However both
  chain-related and wallet-related RPCs are available via
  [lddlwallet](https://github.com/Legenddigital/lddlwallet).
* lddld is secure by default which means that the RPC connection is TLS-enabled
  by default
* lddld provides access to the API through both
  [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29) requests and
  [Websockets](http://en.wikipedia.org/wiki/WebSocket)

Websockets are the preferred transport for lddld RPC and are used by applications
such as [lddlwallet](https://github.com/Legenddigital/lddlwallet) for inter-process
communication with lddld.  The websocket connection endpoint for lddld is
...
during the development of lddlwallet.
...
The lddld RPC server supports both [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29)
...
lddld is `wss://your_ip_or_domain:9109/ws`.
...
to a lddld RPC server:

* **rpcuser** is the full-access username configured for the lddld RPC server
* **rpcpass** is the full-access password configured for the lddld RPC server
* **rpclimituser** is the limited username configured for the lddld RPC server
* **rpclimitpass** is the limited password configured for the lddld RPC server
* **rpccert** is the PEM-encoded X.509 certificate (public key) that the lddld
  server is configured with.  It is automatically generated by lddld and placed
  in the lddld home directory (which is typically `%LOCALAPPDATA%\Lddld` on
  Windows and `~/.lddld` on POSIX-like OSes)

**NOTE:** As mentioned above, lddld is secure by default which means the RPC
...
The lddld RPC server uses HTTP [basic access authentication](http://en.wikipedia.org/wiki/Basic_access_authentication) with the **rpcuser**
...
lddld comes with a separate utility named `lddlctl` which can be used to issue
these RPC commands via HTTP POST requests to lddld after configuring it with the
...
API compatible with the original lddld client.
...
|24|[getwork](#getwork)|N|Returns formatted hash data to work on or checks and submits solved data.<br /><br />NOTE: Since lddld does not have the wallet integrated to provide payment addresses, lddld must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
...
|27|[sendrawtransaction](#sendrawtransaction)|Y|Submits the serialized, hex-encoded transaction to the local peer and relays it to the network.<br /><br />NOTE: lddld does not yet implement the `allowhighfees` parameter, so it has no effect|
|28|[setgenerate](#setgenerate) |N|Set the server to generate coins (mine) or not.<br/>NOTE: Since lddld does not have the wallet integrated to provide payment addresses, lddld must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|29|[stop](#stop)|N|Shutdown lddld.|
|30|[submitblock](#submitblock)|Y|Attempts to submit a new serialized, hex-encoded block to the network.|
|31|[validateaddress](#validateaddress)|Y|Verifies the given address is valid.  NOTE: Since lddld does not have a wallet integrated, lddld will only return whether the address is valid or not.|
...
|35|[getcurrentnet](#getcurrentnet)|Y|Get Legenddigital network lddld is running on.|
...
|Example Return|`[{"addr": "178.172.xxx.xxx:9108", "services": "00000001", "lastrecv": 1388183523, "lastsend": 1388185470, "bytessent": 287592965, "bytesrecv": 780340, "conntime": 1388182973, "pingtime": 405551, "pingwait": 183023, "version": 70001, "subver": "/lddld:0.4.0/", "inbound": false, "startingheight": 276921, "currentheight": 276955, "syncnode": true }, ...]`|
[Return to Overview](#MethodOverview)<br />
...
|Description|Returns information about a transaction given its hash.|
|Notes|Since lddld does not have the wallet integrated to provide payment addresses, lddld must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.
...
|Notes|NOTE: Since lddld does not have the wallet integrated to provide payment addresses, lddld must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|Returns|Nothing|
[Return to Overview](#MethodOverview)<br />
...
|Notes|lddld does not yet implement the `allowhighfees` parameter, so it has no effect.|
...
|Method|stop|
|Parameters|None|
|Description|Shutdown lddld.|
|Returns|`"lddld stopping."` (string)|
[Return to Overview](#MethodOverview)<br />
...
|Parameters|1. `address`: `(string, required)` Legenddigital address.|
|Description|Verify an address is valid.|
|Returns|`(json object)`<br />`isvalid`: `(bool)` whether or not the address is valid.<br />`address`: `(string)` the Legenddigital address validated.<br /><br />`{"isvalid": true or false,"address": "Legenddigitaladdress"}`|
[Return to Overview](#MethodOverview)<br />
...
|Description|Verifies the block chain database. The actual checks performed by the `checklevel` parameter is implementation specific. <br /><br />For lddld this is: <br />`checklevel=0` - Look up each block and ensure it can be loaded from the database.<br />`checklevel=1` - Perform basic context-free sanity checks on each block.|
|Notes|Lddld currently only supports `checklevel` 0 and 1, but the default is still 3 for compatibility.  Per the information in the Parameters section above, higher levels are automatically clamped to the highest supported level, so this means the default is effectively 1 for lddld.|
...
|Description|Dynamically changes the debug logging level.<br />The levelspec can either a debug level or of the form `<subsystem>=<level>,<subsystem2>=<level2>,...`<br />The valid debug levels are `trace`, `debug`, `info`, `warn`, `error`, and `critical`.<br />The valid subsystems are `AMGR`, `ADXR`, `BCDB`, `BMGR`, `LDDLD`, `CHAN`, `DISC`, `PEER`, `RPCS`, `SCRP`, `SRVR`, and `TXMP`.<br />Additionally, the special keyword `show` can be used to get a list of the available subsystems.|
|Returns|`string`|
|Example Return|`Done.`|
|Example `show` Return|`Supported subsystems [AMGR ADXR BCDB BMGR LDDLD CHAN DISC PEER RPCS SCRP SRVR TXMP]`|
...
|Parameters|None|
|Description|Get Legenddigital network lddld is running on.|
|Returns|`numeric`|
...
|Parameters|1. `Addresses`: `(JSON array, required)`<br />`Legenddigitaladdress`: `(string)` the Legenddigital address.<br /><br />`["Legenddigitaladdress", ...]`|
...
|Description|Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this lddld instance) and when such a transaction first appears in a newly-attached block.|
|Returns|Nothing|
[Return to Overview](#WSMethodOverview)<br />
...
lddld uses standard JSON-RPC notifications to notify clients of changes, rather than requiring clients to poll lddld for updates.  JSON-RPC notifications are a subset of requests, but do not contain an ID.  The notification type is categorized by the `method` field and additional details are sent as a JSON array in the `params` field.
...
[rpcclient](https://github.com/Legenddigital/lddld/tree/master/rpcclient) package.
...
[rpcclient](https://github.com/Legenddigital/lddld/tree/master/rpcclient) package to connect with
a lddld instance via Websockets, issues [getblockcount](#getblockcount) to
...
func main() {
	...
	// generated by lddld when it starts the RPC server and doesn't already
	// have one.
...
The following is an example Go application which uses the
[rpcclient](https://github.com/Legenddigital/lddld/tree/master/rpcclient) package to connect with
a lddld instance via Websockets, issues [getblock](#getblock) to retrieve
...
The following is an example Go application which uses the
[rpcclient](https://github.com/Legenddigital/lddld/tree/master/rpcclient) package to connect with
a lddld instance via Websockets and registers for
...
(can be installed with `npm install ws`) to connect with a lddld instance,
...
// generated by lddld when it starts the RPC server and doesn't already
// have one.
var cert = fs.readFileSync('/path/to/lddld/appdata/rpc.cert');
var user = "yourusername";
var password = "yourpassword";


// Initiate the websocket connection.  The lddld generated certificate acts as
