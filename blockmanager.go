...
// blockMsg packages a Legenddigital block message and the peer it came from together
...
// invMsg packages a Legenddigital inv message and the peer it came from together
// so the block handler has access to that information.
type invMsg struct {
	inv  *wire.MsgInv
	peer *serverPeer
...
// headersMsg packages a Legenddigital headers message and the peer it came from
...
// txMsg packages a Legenddigital tx message and the peer it came from together
// so the block handler has access to that information.
type txMsg struct {
	tx   *lddlutil.Tx
	peer *serverPeer
...
func (b *blockManager) checkBlockForHiddenVotes(block *lddlutil.Block) {
	var votesFromBlock []*lddlutil.Tx
	...
	var newVotes []*lddlutil.Tx
	var oldTickets []*lddlutil.Tx
	var oldRevocations []*lddlutil.Tx
	oldVoteMap := make(map[chainhash.Hash]struct{},
		int(b.server.chainParams.TicketsPerBlock))
	if template != nil {
		templateBlock := lddlutil.NewBlock(template.Block)
	...
	updatedTxTreeStake := make([]*lddlutil.Tx, 0,
	...
	var updatedTxTreeRegular []*lddlutil.Tx
...
func (b *blockManager) handleNotifyMsg(notification *blockchain.Notification) {
	...
	case blockchain.NTBlockConnected:
		blockSlice, ok := notification.Data.([]*lddlutil.Block)
		if !ok {
		...
		// below.
		txTreeRegularValid := lddlutil.IsFlagSet16(block.MsgBlock().Header.VoteBits,
			lddlutil.BlockValid)
...
// queue.
func (b *blockManager) QueueTx(tx *lddlutil.Tx, sp *serverPeer) {
...
func (b *blockManager) QueueBlock(block *lddlutil.Block, sp *serverPeer) {
...
func (b *blockManager) ProcessBlock(block *lddlutil.Block, flags blockchain.BehaviorFlags) (bool, error) {
...
func (b *blockManager) ProcessTransaction(tx *lddlutil.Tx, allowOrphans bool,
	rateLimit bool, allowHighFees bool) ([]*lddlutil.Tx, error) {
...
// pool.
func (b *blockManager) TicketPoolValue() (lddlutil.Amount, error) {
...
// newBlockManager returns a new Legenddigital block manager.
...
func warnMultipleDBs() {
	...
	if len(duplicateDbPaths) > 0 {
		...
		lddldLog.Warnf("WARNING: There are multiple block chain databases "+
...
func loadBlockDB() (database.DB, error) {
	...
	if cfg.DbType == "memdb" {
		lddldLog.Infof("Creating block database in memory.")
	...
	lddldLog.Infof("Loading block database from '%s'", dbPath)
	...
	lddldLog.Info("Block database loaded")
	return db, nil
}
