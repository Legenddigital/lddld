...
/*
Package wire implements the Legenddigital wire protocol.

For the complete details of the Legenddigital protocol, see the official wiki entry
...
supported Legenddigital messages to and from the wire.  This package does not deal
with the specifics of message handling such as what to do when a message is
received.  This provides the caller with a high level of flexibility.

Legenddigital Message Overview

The Legenddigital protocol consists of exchanging messages between peers.  Each
message is preceded by a header which identifies information about it such as
which Legenddigital network it is a part of, its type, how big it is, and a checksum
...
of the currently supported Legenddigital messages are provided.  For these supported
messages, all of the details of marshalling and unmarshalling to and from the
wire using Legenddigital encoding are handled so the caller doesn't have to concern
themselves with the specifics.

Message Interaction

The following provides a quick summary of how the Legenddigital messages are intended
to interact with one another.  As stated above, these interactions are not
directly handled by this package.  For more in-depth details about the
appropriate interactions, see the official Legenddigital protocol wiki entry at
...
and write Legenddigital messages.  The following sections provide a quick overview of
...
Legenddigital Network

The Legenddigital network is a magic number which is used to identify the start of a
message and which Legenddigital network the message applies to.  This package provides
...
As discussed in the Legenddigital message overview section, this package reads
and writes Legenddigital messages using a generic interface named Message.  In
...
In order to unmarshall Legenddigital messages from the wire, use the ReadMessage
function.  It accepts any io.Reader, but typically this will be a net.Conn to
a remote node running a Legenddigital peer.  Example syntax is:

	// Reads and validates the next Legenddigital message from conn using the
	// protocol version pver and the Legenddigital network btcnet.  The returns
...
In order to marshall Legenddigital messages to the wire, use the WriteMessage
function.  It accepts any io.Writer, but typically this will be a net.Conn to
a remote node running a Legenddigital peer.  Example syntax to request addresses
from a remote peer is:
	...
	// pver, and the Legenddigital network btcnet.  The return is a possible
	// error.
