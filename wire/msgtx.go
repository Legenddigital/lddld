...
// OutPoint defines a Legenddigital data type that is used to track previous
// transaction outputs.
...
// NewOutPoint returns a new Legenddigital transaction outpoint point with the
...
// NewTxIn returns a new Legenddigital transaction input with the provided
...
// NewTxOut returns a new Legenddigital transaction output with the provided
...
// MsgTx implements the Message interface and represents a Legenddigital tx message.
...
func writeTxScriptsToMsgTx(msg *MsgTx, totalScriptSize uint64, serType TxSerializeType) {
	...
	//
	// Closures around writing the TxIn and TxOut scripts are used in Legenddigital
...
// BtcDecode decodes r using the Legenddigital protocol encoding into the receiver.
...
// in that BtcDecode decodes from the Legenddigital wire protocol as it was sent
...
// BtcEncode encodes the receiver to w using the Legenddigital protocol encoding.
// This is part of the Message interface implementation.
...
// encodes the transaction to the Legenddigital wire protocol in order to be sent
// across the network.  The wire encoding can technically differ depending on
// the protocol version and doesn't even really need to match the format of a
// stored transaction at all.  As of the time this comment was written, the
// encoded transaction is the same in both instances, but there is a distinct
...
// NewMsgTx returns a new Legenddigital tx message that conforms to the Message
...
// WriteOutPoint encodes op to the Legenddigital protocol encoding for an OutPoint
// to w.
...
// writeTxInPrefixs encodes ti to the Legenddigital protocol encoding for a transaction
...
// writeTxWitness encodes ti to the Legenddigital protocol encoding for a transaction
...
// writeTxOut encodes to into the Legenddigital protocol encoding for a transaction
// output (TxOut) to w.
