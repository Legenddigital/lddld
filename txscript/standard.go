...
func payToPubKeyHashEdwardsScript(pubKeyHash []byte) ([]byte, error) {
	edwardsData := []byte{byte(lddlec.STEd25519)}
	return NewScriptBuilder().AddOp(OP_DUP).AddOp(OP_HASH160).
...
func payToPubKeyHashSchnorrScript(pubKeyHash []byte) ([]byte, error) {
	schnorrData := []byte{byte(lddlec.STSchnorrSecp256k1)}
	return NewScriptBuilder().AddOp(OP_DUP).AddOp(OP_HASH160).
...
// valid SStxs.
func PayToSStx(addr lddlutil.Address) ([]byte, error) {
	...
	case *lddlutil.AddressPubKeyHash:
		...
		if addr.DSA(addr.Net()) != lddlec.STEcdsaSecp256k1 {
...
// valid SStxs.
func PayToSStxChange(addr lddlutil.Address) ([]byte, error) {
	// Only pay to pubkey hash and pay to script hash are
	// supported.
	scriptType := PubKeyHashTy
	switch addr := addr.(type) {
...
// valid SSGen txs.
func PayToSSGen(addr lddlutil.Address) ([]byte, error) {
	// Only pay to pubkey hash and pay to script hash are
	// supported.
	scriptType := PubKeyHashTy
	switch addr := addr.(type) {
...
// valid SSRtx.
func PayToSSRtx(addr lddlutil.Address) ([]byte, error) {
	// Only pay to pubkey hash and pay to script hash are
	// supported.
	scriptType := PubKeyHashTy
	switch addr := addr.(type) {
...
// an SStx.
func GenerateSStxAddrPush(addr lddlutil.Address, amount lddlutil.Amount, limits uint16) ([]byte, error) {
	// Only pay to pubkey hash and pay to script hash are
	// supported.
	scriptType := PubKeyHashTy
	switch addr := addr.(type) {
...
// specified address.
func PayToAddrScript(addr lddlutil.Address) ([]byte, error) {
	switch addr := addr.(type) {
	case *lddlutil.AddressPubKeyHash:
		if addr == nil {
			return nil, scriptError(ErrUnsupportedAddress,
	...
	case *lddlutil.AddressEdwardsPubKey:
		if addr == nil {
			return nil, scriptError(ErrUnsupportedAddress,
				nilAddrErrStr)
		}
	...
	case *lddlutil.AddressSecSchnorrPubKey:
		if addr == nil {
			return nil, scriptError(ErrUnsupportedAddress,
				nilAddrErrStr)
		}
...
func MultiSigScript(pubkeys []*lddlutil.AddressSecpPubKey, nrequired int) ([]byte, error) {
...
func ExtractPkScriptAddrs(version uint16, pkScript []byte,
	chainParams *chaincfg.Params) (ScriptClass, []lddlutil.Address, int, error) {
	if version != DefaultScriptVersion {
	...
	case PubKeyHashTy:
		...
		requiredSigs = 1
		addr, err := lddlutil.NewAddressPubKeyHash(pops[2].data,
			chainParams, lddlec.STEcdsaSecp256k1)
		if err == nil {
	...
	case PubKeyTy:
		...
		if err == nil {
			addr, err := lddlutil.NewAddressSecpPubKeyCompressed(pk, chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
		}
	...
	case PubkeyAltTy:
		...
		switch suite {
		case lddlec.STEd25519:
			addr, err = lddlutil.NewAddressEdwardsPubKey(pops[0].data,
				chainParams)
		case lddlec.STSchnorrSecp256k1:
			addr, err = lddlutil.NewAddressSecSchnorrPubKey(pops[0].data,
				chainParams)
		}
		if err == nil {
			addrs = append(addrs, addr)
	...
	case StakeSubmissionTy:
		...
		var localAddrs []lddlutil.Address
	...
	case ScriptHashTy:
		...
		requiredSigs = 1
		addr, err := lddlutil.NewAddressScriptHashFromHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case MultiSigTy:
		...
		addrs = make([]lddlutil.Address, 0, numPubKeys)
...
func ExtractPkScriptAltSigType(pkScript []byte) (lddlec.SignatureType, error) {
	pops, err := parseScript(pkScript)
	if err != nil {
		return 0, err
	}
	...
	val := lddlec.SignatureType(valInt)
	switch val {
	case lddlec.STEd25519:
		return val, nil
	case lddlec.STSchnorrSecp256k1:
...
//
// NOTE: Atomic swaps are not considered standard script types by the lddld
