...
// This example demonstrates creating a script which pays to a Legenddigital address.
// It also prints the created script hex and uses the DisasmString function to
// display the disassembled script.
func ExamplePayToAddrScript() {
	// Parse the address to send the coins to into a lddlutil.Address
	// which is useful to ensure the accuracy of the address and determine
	// the address type.  It is also required for the upcoming call to
	// PayToAddrScript.
	addressStr := "DsSej1qR3Fyc8kV176DCh9n9cY9nqf9Quxk"
	address, err := lddlutil.DecodeAddress(addressStr)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func ExampleSignTxOutput() {
	...
	addr, err := lddlutil.NewAddressPubKeyHash(pubKeyHash,
		&chaincfg.MainNetParams, lddlec.STEcdsaSecp256k1)
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	// contains a single output that pays to address in the amount of 1 LDDL.
	...
	lookupKey := func(a lddlutil.Address) (chainec.PrivateKey, bool, error) {
