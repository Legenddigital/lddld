...
func RawTxInSignatureAlt(tx *wire.MsgTx, idx int, subScript []byte,
	hashType SigHashType, key chainec.PrivateKey, sigType lddlec.SignatureType) ([]byte,
	error) {
...
func SignatureScriptAlt(tx *wire.MsgTx, idx int, subscript []byte,
	...
	sigType lddlec.SignatureType) ([]byte,
	error) {
...
func p2pkSignatureScriptAlt(tx *wire.MsgTx, idx int, subScript []byte,
	hashType SigHashType, privKey chainec.PrivateKey, sigType lddlec.SignatureType) ([]byte,
	error) {
	sig, err := RawTxInSignatureAlt(tx, idx, subScript, hashType, privKey,
		sigType)
	if err != nil {
...
func signMultiSig(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType,
	addresses []lddlutil.Address, nRequired int, kdb KeyDB) ([]byte, bool) {
	// No need to add dummy in Legenddigital.
...
func handleStakeOutSign(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	...
	addresses []lddlutil.Address, class ScriptClass, subClass ScriptClass,
	nrequired int) ([]byte, ScriptClass, []lddlutil.Address, int, error) {
...
func sign(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	subScript []byte, hashType SigHashType, kdb KeyDB, sdb ScriptDB,
	sigType lddlec.SignatureType) ([]byte,
	ScriptClass, []lddlutil.Address, int, error) {
...
func mergeScripts(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	pkScript []byte, class ScriptClass, addresses []lddlutil.Address,
...
func mergeMultiSig(tx *wire.MsgTx, idx int, addresses []lddlutil.Address,
...
sigLoop:
	for _, sig := range possibleSigs {
		...
		if err != nil {
			// Legenddigital -- is this the right handling for SIGHASH_SINGLE error ?
		...
		for _, addr := range addresses {
			...
			// bad input.
			pkaddr := addr.(*lddlutil.AddressSecpPubKey)
...
type KeyDB interface {
	GetKey(lddlutil.Address) (chainec.PrivateKey, bool, error)
}

// KeyClosure implements KeyDB with a closure.
type KeyClosure func(lddlutil.Address) (chainec.PrivateKey, bool, error)

// GetKey implements KeyDB by returning the result of calling the closure.
func (kc KeyClosure) GetKey(address lddlutil.Address) (chainec.PrivateKey,
	bool, error) {
	return kc(address)
}
...
type ScriptDB interface {
	GetScript(lddlutil.Address) ([]byte, error)
}

// ScriptClosure implements ScriptDB with a closure.
type ScriptClosure func(lddlutil.Address) ([]byte, error)

// GetScript implements ScriptDB by returning the result of calling the closure.
func (sc ScriptClosure) GetScript(address lddlutil.Address) ([]byte, error) {
	return sc(address)
}
...
func SignTxOutput(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	...
	previousScript []byte, sigType lddlec.SignatureType) ([]byte, error) {
