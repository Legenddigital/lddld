...
func mkGetKey(keys map[string]addressToKey) KeyDB {
	if keys == nil {
		return KeyClosure(func(addr lddlutil.Address) (chainec.PrivateKey,
			bool, error) {
...
func mkGetScript(scripts map[string][]byte) ScriptDB {
	if scripts == nil {
		return ScriptClosure(func(addr lddlutil.Address) (
			[]byte, error) {
			return nil, errors.New("nope 3")
		})
	}
	return ScriptClosure(func(addr lddlutil.Address) ([]byte,
		error) {
...
func signAndCheck(msg string, tx *wire.MsgTx, idx int, pkScript []byte,
	...
	previousScript []byte, suite lddlec.SignatureType) error {
...
func TestSignTxOutput(t *testing.T) {
	...
	signatureSuites := []lddlec.SignatureType{
	...
	for _, hashType := range hashTypes {
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				address, err := lddlutil.NewAddressPubKeyHash(
					lddlutil.Hash160(pkBytes), testingParams,
					suite)
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			if err := signAndCheck(msg, tx, i, pkScript, hashType,
				...
				lddlec.STEcdsaSecp256k1); err != nil {
				t.Error(err)
				break
			}
	...
	for _, hashType := range hashTypes {
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				// for more details
				address, err := lddlutil.NewAddressSecpPubKeyCompressed(pk,
					testingParams)
				if err != nil {
					t.Errorf("failed to make address for %s: %v",
						msg, err)
		...
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				case lddlec.STEd25519:
					...
					key, pk = chainec.Edwards.PrivKeyFromBytes(keyDB)
					pkBytes = pk.SerializeUncompressed()
					address, err = lddlutil.NewAddressEdwardsPubKey(pkBytes,
						testingParams)
					if err != nil {
						t.Errorf("failed to make address for %s: %v",
							msg, err)
				...
				case lddlec.STSchnorrSecp256k1:
					...
					key, pk = chainec.SecSchnorr.PrivKeyFromBytes(keyDB)
					pkBytes = pk.Serialize()
					address, err = lddlutil.NewAddressSecSchnorrPubKey(pkBytes,
						testingParams)
					if err != nil {
						t.Errorf("failed to make address for %s: %v",
							msg, err)
	...
	for _, hashType := range hashTypes {
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				address, err := lddlutil.NewAddressPubKeyHash(
					lddlutil.Hash160(pkBytes), testingParams, suite)
				if err != nil {
					t.Errorf("failed to make address for %s: %v",
						msg, err)
					break
				...
				scriptAddr, err := lddlutil.NewAddressScriptHash(
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			address1, err := lddlutil.NewAddressSecpPubKeyCompressed(pk1,
				testingParams)
			if err != nil {
				t.Errorf("failed to make address for %s: %v",
					msg, err)
			...
			address2, err := lddlutil.NewAddressSecpPubKeyCompressed(pk2,
				testingParams)
			if err != nil {
			...
			pkScript, err := MultiSigScript(
				[]*lddlutil.AddressSecpPubKey{address1, address2},
				2)
			if err != nil {
				t.Errorf("failed to make pkscript "+
					"for %s: %v", msg, err)
			...
			if err := signAndCheck(msg, tx, i, scriptPkScript,
				...
				mkGetKey(map[string]addressToKey{
					...
					scriptAddr.EncodeAddress(): pkScript,
				}), nil, lddlec.STEcdsaSecp256k1); err != nil {
				t.Error(err)
				break
			}
...
var (
	...
	thisAddressUnc, _ = lddlutil.NewAddressPubKeyHash(
		lddlutil.Hash160(thisPubKey.SerializeUncompressed()),
		testingParams, lddlec.STEcdsaSecp256k1)
	uncompressedPkScript, _ = PayToAddrScript(thisAddressUnc)
	thisAddressCom, _       = lddlutil.NewAddressPubKeyHash(
		lddlutil.Hash160(thisPubKey.SerializeCompressed()),
		testingParams, lddlec.STEcdsaSecp256k1)
