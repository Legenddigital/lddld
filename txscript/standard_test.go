...
// newAddressPubKey returns a new lddlutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) lddlutil.Address {
	pubkey, err := secp256k1.ParsePubKey(serializedPubKey)
	if err != nil {
		panic("invalid public key in test source")
	}
	addr, err := lddlutil.NewAddressSecpPubKeyCompressed(pubkey,
...
// newAddressPubKeyHash returns a new lddlutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) lddlutil.Address {
	addr, err := lddlutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams,
...
// newAddressScriptHash returns a new lddlutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) lddlutil.Address {
	addr, err := lddlutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the lddlutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the lddlutil.Address interface.
...
// DSA returns -1.
func (b *bogusAddress) DSA(chainParams *chaincfg.Params) lddlec.SignatureType {
	return -1
}
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := lddlutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := lddlutil.NewAddressScriptHashFromHash(hexToBytes("e8c30"+
	...
	p2pkCompressedMain, err := lddlutil.NewAddressSecpPubKey(hexToBytes("02192d7"+
	...
	p2pkCompressed2Main, err := lddlutil.NewAddressSecpPubKey(hexToBytes("03b0b"+
	...
	tests := []struct {
		...
		// pay-to-pubkey address on mainnet. for Legenddigital this would
		// be uncompressed, but standard for Legenddigital is 33 byte
		...
		{(*lddlutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*lddlutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*lddlutil.AddressSecpPubKey)(nil), "", errUnsupportedAddress},
		{(*lddlutil.AddressEdwardsPubKey)(nil), "", errUnsupportedAddress},
		{(*lddlutil.AddressSecSchnorrPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	//  mainnet p2pk 13CG6SJ3yHUXo4Cr2RY4THLLJrNFuG3gUg
	p2pkCompressedMain, err := lddlutil.NewAddressSecpPubKey(hexToBytes("02192d"+
	...
	tests := []struct {
		keys      []*lddlutil.AddressSecpPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*lddlutil.AddressSecpPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
		...
		{
			// By default compressed pubkeys are used in Legenddigital.
			[]*lddlutil.AddressSecpPubKey{
				p2pkUncompressedMain.(*lddlutil.AddressSecpPubKey),
			},
			1,
