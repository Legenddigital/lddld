...
import (
	...
	_ "github.com/Legenddigital/lddld/database/ffldb"
	"github.com/Legenddigital/lddld/lddlutil"
	"github.com/Legenddigital/lddld/mempool"
	"github.com/Legenddigital/lddld/sampleconfig"
...
const (
	defaultConfigFilename        = "lddld.conf"
	defaultDataDirname           = "data"
	defaultLogLevel              = "info"
	defaultLogDirname            = "logs"
	defaultLogFilename           = "lddld.log"
	defaultMaxPeers              = 125
...
var (
	defaultHomeDir     = lddlutil.AppDataDir("lddld", false)
...
type config struct {
	...
	MinRelayTxFee        float64       `long:"minrelaytxfee" description:"The minimum transaction fee in LDDL/kB to be considered a non-zero fee."`
...
// createDefaultConfig copies the file sample-lddld.conf to the given destination path,
...
//
// The above results in lddld functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	// Update the home directory for lddld if specified. Since the home
	...
	cfg.minRelayTxFee, err = lddlutil.NewAmount(cfg.MinRelayTxFee)
	if err != nil {
	...
	cfg.miningAddrs = make([]lddlutil.Address, 0, len(cfg.GetWorkKeys)+
		len(cfg.MiningAddrs))
	for _, strAddr := range cfg.GetWorkKeys {
		addr, err := lddlutil.DecodeAddress(strAddr)
		if err != nil {
	...
	for _, oldDir := range oldTestNets {
		if fileExists(oldDir) {
			lddldLog.Warnf("Block chain data from previous testnet"+
	...
	if configFileError != nil {
		lddldLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// lddldDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func lddldDial(network, addr string) (net.Conn, error) {
...
// lddldLookup returns the correct DNS lookup function to use depending on the
...
func lddldLookup(host string) ([]net.IP, error) {
