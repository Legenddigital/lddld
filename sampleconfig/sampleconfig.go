...
// FileContents is a string containing the commented example config for lddld.
...
; The default is ~/.lddld/data on POSIX OSes, $LOCALAPPDATA/Lddld/data on Windows,
; ~/Library/Application Support/Lddld/data on macOS, and $homed/lddld/data on
; Plan9.  Environment variables are expanded so they may be used.  NOTE: Windows
; environment variables are typically %VARIABLE%, but they must be accessed with
; $VARIABLE here.
; datadir=~/.lddld/data                            ; Unix
; datadir=$LOCALAPPDATA/Lddld/data                 ; Windows
; datadir=~/Library/Application Support/Lddld/data ; macOS
...
; line.  lddld will not contact 3rd-party sites to obtain external ip addresses.
...
; with, but the behavior is slightly different.  By default, lddld will query DNS
...
; Disable DNS seeding for peers.  By default, when lddld starts, it will use
...
; which is used to control and query information from a running lddld process.
;
...
; log level for individual subsystems.  Use lddld --debuglevel=show to list
