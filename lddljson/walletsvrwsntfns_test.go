...
func TestWalletSvrWsNtfns(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name: "lddldconnected",
			newNtfn: func() (interface{}, error) {
				return NewCmd("lddldconnected", true)
			},
			staticNtfn: func() interface{} {
				return NewLddldConnectedNtfn(true)
			},
			marshalled: `{"jsonrpc":"1.0","method":"lddldconnected","params":[true],"id":null}`,
			unmarshalled: &LddldConnectedNtfn{
				Connected: true,
			},
		},
		{
