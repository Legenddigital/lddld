...
var (
	...
	// data.
	gbtCoinbaseAux = &lddljson.GetBlockTemplateResultAux{
...
var (
	...
	ErrRPCUnimplemented = &lddljson.RPCError{
	...
	ErrRPCNoWallet = &lddljson.RPCError{
		Code:    lddljson.ErrRPCNoWallet,
...
// list of commands that we recognize, but for which lddld has no support because
// it lacks support for wallet functionality. For these commands the user
// should ask a connected instance of lddlwallet.
...
// not needed.
func rpcInternalError(errStr, context string) *lddljson.RPCError {
	logStr := errStr
	if context != "" {
		logStr = context + ": " + errStr
	}
	rpcsLog.Error(logStr)
	return lddljson.NewRPCError(lddljson.ErrRPCInternal.Code, errStr)
}
...
func rpcInvalidError(fmtStr string, args ...interface{}) *lddljson.RPCError {
	return lddljson.NewRPCError(lddljson.ErrRPCInvalidParameter,
...
func rpcDeserializationError(fmtStr string, args ...interface{}) *lddljson.RPCError {
	return lddljson.NewRPCError(lddljson.ErrRPCDeserialization,
		fmt.Sprintf(fmtStr, args...))
}

// rpcRuleError is a convenience function to convert a
// rule error to an RPC error with the appropriate code set.
func rpcRuleError(fmtStr string, args ...interface{}) *lddljson.RPCError {
	return lddljson.NewRPCError(lddljson.ErrRPCMisc,
		fmt.Sprintf(fmtStr, args...))
}
...
func rpcDuplicateTxError(fmtStr string, args ...interface{}) *lddljson.RPCError {
	return lddljson.NewRPCError(lddljson.ErrRPCDuplicateTx,
		fmt.Sprintf(fmtStr, args...))
}
...
// context parameter is only used in the log message and may be empty if it's
// not needed.
func rpcAddressKeyError(fmtStr string, args ...interface{}) *lddljson.RPCError {
	return lddljson.NewRPCError(lddljson.ErrRPCInvalidAddressOrKey,
		fmt.Sprintf(fmtStr, args...))
}

// rpcDecodeHexError is a convenience function for returning a nicely formatted
// RPC error which indicates the provided hex string failed to decode.
func rpcDecodeHexError(gotHex string) *lddljson.RPCError {
	return lddljson.NewRPCError(lddljson.ErrRPCDecodeHexString,
...
// transaction hash.
func rpcNoTxInfoError(txHash *chainhash.Hash) *lddljson.RPCError {
	return lddljson.NewRPCError(lddljson.ErrRPCNoTxInfo,
...
func rpcMiscError(message string) *lddljson.RPCError {
	return lddljson.NewRPCError(lddljson.ErrRPCMisc, message)
}
...
// These commands will be implemented in lddlwallet.
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*lddljson.CreateRawTransactionCmd)
	...
	for _, input := range c.Inputs {
		...
		if input.Amount > 0 {
			amt, err := lddlutil.NewAmount(input.Amount)
			if err != nil {
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount > lddlutil.MaxAmount {
			return nil, rpcInvalidError("Invalid amount: 0 >= %v "+
				"> %v", amount, lddlutil.MaxAmount)
		}

		// Decode the provided address.
		addr, err := lddlutil.DecodeAddress(encodedAddr)
		if err != nil {
		...
		atomic, err := lddlutil.NewAmount(amount)
		if err != nil {
			return nil, rpcInternalError(err.Error(),
				"New amount")
		}
...
func handleCreateRawSStx(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*lddljson.CreateRawSStxCmd)
	...
	for i, cout := range c.COuts {
		...
		addr, err := lddlutil.DecodeAddress(cout.Addr)
		if err != nil {
			return nil, rpcAddressKeyError("Could not decode "+
				"address: %v", err)
		}
		...
		pkScript, err := txscript.GenerateSStxAddrPush(addr,
			lddlutil.Amount(amountsCommitted[i]), 0x0000)
		if err != nil {
			return nil, rpcInternalError(err.Error(),
		...
		// Ensure amount is in the valid range for monetary amounts.
		if cout.ChangeAmt < 0 || cout.ChangeAmt > lddlutil.MaxAmount {
			return nil, rpcInvalidError("Invalid change amount: 0 "+
				"> %v > %v", cout.ChangeAmt, lddlutil.MaxAmount)
		}

		// Decode the provided address.
		addr, err = lddlutil.DecodeAddress(cout.ChangeAddr)
		if err != nil {
			return nil, rpcAddressKeyError("Wrong network: %v",
				addr)
		}
...
func handleCreateRawSSGenTx(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*lddljson.CreateRawSSGenTxCmd)
	...
	for i, ssgenPkh := range ssgenPkhs {
		...
		if ssgenCalcAmts[i] <= 0 ||
			ssgenCalcAmts[i] > lddlutil.MaxAmount {
...
func handleCreateRawSSRtx(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*lddljson.CreateRawSSRtxCmd)
	...
	var feeAmt lddlutil.Amount
	if c.Fee != nil {
		var err error
		feeAmt, err = lddlutil.NewAmount(*c.Fee)
		if err != nil {
	...
	for i, ssrtxPkh := range ssrtxPkhs {
		// Ensure amount is in the valid range for monetary amounts.
		if sstxAmts[i] <= 0 || sstxAmts[i] > lddlutil.MaxAmount {
			return nil, rpcInvalidError("Invalid SSTx amount: 0 >="+
				" %v > %v", sstxAmts[i] <= 0, lddlutil.MaxAmount)
		}
...
// transaction.
func createVinList(mtx *wire.MsgTx) []lddljson.Vin {
	// Coinbase transactions only have a single txin by definition.
	vinList := make([]lddljson.Vin, len(mtx.TxIn))
	if blockchain.IsCoinBaseTx(mtx) {
		txIn := mtx.TxIn[0]
		vinEntry := &vinList[0]
		vinEntry.Coinbase = hex.EncodeToString(txIn.SignatureScript)
		vinEntry.Sequence = txIn.Sequence
		vinEntry.AmountIn = lddlutil.Amount(txIn.ValueIn).ToCoin()
	...
	for i, txIn := range mtx.TxIn {
		...
		vinEntry.BlockHeight = txIn.BlockHeight
		vinEntry.BlockIndex = txIn.BlockIndex
		vinEntry.ScriptSig = &lddljson.ScriptSig{
			Asm: disbuf,
...
// transaction.
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []lddljson.Vout {

	txType := stake.DetermineTxType(mtx)
	voutList := make([]lddljson.Vout, 0, len(mtx.TxOut))
	for i, v := range mtx.TxOut {
		...
		var vout lddljson.Vout
		voutSPK := &vout.ScriptPubKey
		vout.N = uint32(i)
		vout.Value = lddlutil.Amount(v.Value).ToCoin()
		...
		if commitAmt != nil {
			voutSPK.CommitAmt = lddljson.Float64(commitAmt.ToCoin())
		}
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx, txHash string, blkIdx uint32, blkHeader *wire.BlockHeader, blkHash string, blkHeight int64, confirmations int64) (*lddljson.TxRawResult, error) {
	mtxHex, err := messageToHex(mtx)
	if err != nil {
		return nil, err
	}
...
func handleDecodeRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*lddljson.DecodeRawTransactionCmd)
	...
	txReply := lddljson.TxRawDecodeResult{
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*lddljson.DecodeScriptCmd)
	...
	p2sh, err := lddlutil.NewAddressScriptHash(script, s.server.chainParams)
	if err != nil {
		return nil, rpcInternalError(err.Error(),
	...
	reply := lddljson.DecodeScriptResult{
		Asm:       disbuf,
...
func handleEstimateStakeDiff(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*lddljson.EstimateStakeDiffCmd)
	...
	if c.Tickets != nil {
		...
		userEstFlt := lddlutil.Amount(userEst).ToCoin()
		userEstFltPtr = &userEstFlt
	}

	return &lddljson.EstimateStakeDiffResult{
		Min:      lddlutil.Amount(min).ToCoin(),
		Max:      lddlutil.Amount(max).ToCoin(),
		Expected: lddlutil.Amount(expected).ToCoin(),
		User:     userEstFltPtr,
	}, nil
}
...
func handleExistsAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*lddljson.ExistsAddressCmd)

	// Attempt to decode the supplied address.
	addr, err := lddlutil.DecodeAddress(c.Address)
	if err != nil {
...
func handleExistsAddresses(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*lddljson.ExistsAddressesCmd)
	addresses := make([]lddlutil.Address, len(c.Addresses))
	for i := range c.Addresses {
		// Attempt to decode the supplied address.
		addr, err := lddlutil.DecodeAddress(c.Addresses[i])
		if err != nil {
			return nil, rpcAddressKeyError("Could not decode "+
				"address: %v", err)
		}
...
func handleExistsMissedTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*lddljson.ExistsMissedTicketsCmd)

	hashes, err := lddljson.DecodeConcatenatedHashes(c.TxHashBlob)
	if err != nil {
		return nil, err
	}
...
func handleExistsExpiredTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*lddljson.ExistsExpiredTicketsCmd)
...
func handleExistsLiveTicket(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*lddljson.ExistsLiveTicketCmd)
...
func handleExistsLiveTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*lddljson.ExistsLiveTicketsCmd)
...
func handleExistsMempoolTxs(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*lddljson.ExistsMempoolTxsCmd)
...
func handleGetAddedNodeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*lddljson.GetAddedNodeInfoCmd)

	// Retrieve a list of persistent (added) peers from the Legenddigital server
	...
	results := make([]*lddljson.GetAddedNodeInfoResult, 0, len(peers))
	for _, peer := range peers {
		// Set the "address" of the peer which could be an ip address
		// or a domain name.
		var result lddljson.GetAddedNodeInfoResult
		result.AddedNode = peer.Addr()
		result.Connected = lddljson.Bool(peer.Connected())
		...
		addrs := make([]lddljson.GetAddedNodeInfoResultAddr, 0,
			len(ipList))
		for _, ip := range ipList {
			var addr lddljson.GetAddedNodeInfoResultAddr
			addr.Address = ip
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	sbitsFloat := float64(blockHeader.SBits) / lddlutil.AtomsPerCoin
	blockReply := lddljson.GetBlockVerboseResult{
		Hash:          c.Hash,
	...
	if c.VerboseTx == nil || !*c.VerboseTx {
		...
		rawTxns := make([]lddljson.TxRawResult, len(txns))
		for i, tx := range txns {
		...
		rawSTxns := make([]lddljson.TxRawResult, len(stxns))
...
func handleGetBlockHash(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*lddljson.GetBlockHashCmd)
	hash, err := s.chain.BlockHashByHeight(c.Index)
	if err != nil {
		return nil, &lddljson.RPCError{
			Code: lddljson.ErrRPCOutOfRange,
...
func handleGetBlockHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*lddljson.GetBlockHeaderCmd)
	...
	blockHeaderReply := lddljson.GetBlockHeaderVerboseResult{
		Hash:          c.Hash,
		Confirmations: confirmations,
		Version:       blockHeader.Version,
		PreviousHash:  blockHeader.PrevBlock.String(),
		...
		PoolSize:      blockHeader.PoolSize,
		Bits:          strconv.FormatInt(int64(blockHeader.Bits), 16),
		SBits:         lddlutil.Amount(blockHeader.SBits).ToCoin(),
...
func handleGetBlockSubsidy(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*lddljson.GetBlockSubsidyCmd)
	...
	rep := lddljson.GetBlockSubsidyResult{
		Developer: dev,
		PoS:       pos,
		PoW:       pow,
		Total:     total,
...
// state as a lddljson.GetBlockTemplateResult that is ready to be encoded to
// JSON and returned to the caller.
//
// This function MUST be called with the state locked.
func (state *gbtWorkState) blockTemplateResult(bm *blockManager, useCoinbaseValue bool, submitOld *bool) (*lddljson.GetBlockTemplateResult, error) {
	...
	transactions := make([]lddljson.GetBlockTemplateResultTx, 0, numTx-1)
	...
	for i, tx := range msgBlock.Transactions {
		...
		resultTx := lddljson.GetBlockTemplateResultTx{
	...
	stransactions := make([]lddljson.GetBlockTemplateResultTx, 0, numSTx)
	...
	reply := lddljson.GetBlockTemplateResult{
...
func handleGetBlockTemplateRequest(s *rpcServer, request *lddljson.TemplateRequest, closeChan <-chan struct{}) (interface{}, error) {
	...
	if !cfg.SimNet && s.server.ConnectedCount() == 0 {
		return nil, &lddljson.RPCError{
			...
			Message: "Legenddigital is not connected",
		}
	}
	...
	if currentHeight != 0 && !s.server.blockManager.IsCurrent() {
		return nil, &lddljson.RPCError{
			Code:    lddljson.ErrRPCClientInInitialDownload,
			Message: "Legenddigital is downloading blocks...",
		}
	}
...
// reasons.
// TODO Legenddigital pop in the new errors from blockchain cj
...
func handleGetBlockTemplateProposal(s *rpcServer, request *lddljson.TemplateRequest) (interface{}, error) {
...
func handleGetBlockTemplate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*lddljson.GetBlockTemplateCmd)
	request := c.Request
...
func handleGetCFilter(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	if s.server.cfIndex == nil {
		return nil, &lddljson.RPCError{
			Code:    lddljson.ErrRPCNoCFIndex,
			Message: "Compact filters must be enabled for this command",
		}
	}

	c := cmd.(*lddljson.GetCFilterCmd)
	hash, err := chainhash.NewHashFromStr(c.Hash)
	if err != nil {
		return nil, rpcDecodeHexError(c.Hash)
	}
...
func handleGetCFilterHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*lddljson.GetCFilterHeaderCmd)
	hash, err := chainhash.NewHashFromStr(c.Hash)
	if err != nil {
		return nil, rpcDecodeHexError(c.Hash)
	}
...
func handleGetHeaders(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*lddljson.GetHeadersCmd)
	blockLocators, err := lddljson.DecodeConcatenatedHashes(c.BlockLocators)
	if err != nil {
		// Already a *lddljson.RPCError
		return nil, err
	}
	...
	return &lddljson.GetHeadersResult{Headers: hexBlockHeaders}, nil
}
...
func handleGetMempoolInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &lddljson.GetMempoolInfoResult{
...
func handleGetMiningInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	gnhpsCmd := lddljson.NewGetNetworkHashPSCmd(nil, nil)
	...
	result := lddljson.GetMiningInfoResult{
		Blocks:           best.Height,
...
func handleGetNetTotals(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	reply := &lddljson.GetNetTotalsResult{
...
func handleGetNetworkHashPS(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*lddljson.GetNetworkHashPSCmd)
...
func handleGetPeerInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	infos := make([]*lddljson.GetPeerInfoResult, 0, len(peers))
	for _, p := range peers {
		statsSnap := p.StatsSnapshot()
		info := &lddljson.GetPeerInfoResult{
			ID:             statsSnap.ID,
			Addr:           statsSnap.Addr,
...
func handleGetRawMempool(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*lddljson.GetRawMempoolCmd)
	...
	if c.TxType != nil {
		switch lddljson.GetRawMempoolTxTypeCmd(*c.TxType) {
		case lddljson.GRMRegular:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeRegular
		case lddljson.GRMTickets:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeSStx
		case lddljson.GRMVotes:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeSSGen
		case lddljson.GRMRevocations:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeSSRtx
		case lddljson.GRMAll:
			// Nothing to do
		default:
...
func handleGetRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*lddljson.GetRawTransactionCmd)
...
func handleGetStakeDifficulty(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if err != nil {
		...
		return nil, &lddljson.RPCError{
			Code:    lddljson.ErrRPCDifficulty,
			Message: "Error getting stake difficulty: " + err.Error(),
		}
	}
	currentSdiff := lddlutil.Amount(blockHeader.SBits)
	...
	nextSdiffAmount := lddlutil.Amount(nextSdiff)

	sDiffResult := &lddljson.GetStakeDifficultyResult{
...
func convertVersionMap(m map[int]int) []lddljson.VersionCount {
	sorted := make([]lddljson.VersionCount, 0, len(m))
	...
	for _, v := range order {
		sorted = append(sorted, lddljson.VersionCount{Version: uint32(v),
			Count: uint32(m[v])})
	}
...
func handleGetStakeVersionInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	count := int32(1)
	c, ok := cmd.(*lddljson.GetStakeVersionInfoCmd)
	if !ok {
	...
	result := lddljson.GetStakeVersionInfoResult{
		CurrentHeight: snapshot.Height,
		Hash:          snapshot.Hash.String(),
		Intervals:     make([]lddljson.VersionInterval, 0, count),
	}
	...
	for i := int32(0); i < count; i++ {
		...
		versionInterval := lddljson.VersionInterval{
			StartHeight:  endHeight,
...
func handleGetStakeVersions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*lddljson.GetStakeVersionsCmd)
	...
	result := lddljson.GetStakeVersionsResult{
		StakeVersions: make([]lddljson.StakeVersions, 0, len(sv)),
	}
	for _, v := range sv {
		nsv := lddljson.StakeVersions{
			Hash:         v.Hash.String(),
			Height:       v.Height,
			BlockVersion: v.BlockVersion,
			StakeVersion: v.StakeVersion,
			Votes: make([]lddljson.VersionBits, 0,
				len(v.Votes)),
		}
		for _, vote := range v.Votes {
			nsv.Votes = append(nsv.Votes,
				lddljson.VersionBits{Version: vote.Version,
					Bits: vote.Bits})
		}
...
func handleGetVoteInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c, ok := cmd.(*lddljson.GetVoteInfoCmd)
	if !ok {
		return nil, rpcInvalidError("Invalid type: %T", c)
	}
	...
	// Assemble JSON result.
	result := lddljson.GetVoteInfoResult{
		CurrentHeight: snapshot.Height,
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if txFromMempool != nil {
		...
		if c.Vout > uint32(len(mtx.TxOut)-1) {
			return nil, &lddljson.RPCError{
				Code: lddljson.ErrRPCInvalidTxVout,
	...
	txOutReply := &lddljson.GetTxOutResult{
		BestBlock:     bestBlockHash,
		Confirmations: confirmations,
		Value:         lddlutil.Amount(value).ToUnit(lddlutil.AmountCoin),
		Version:       int32(txVersion),
		ScriptPubKey: lddljson.ScriptPubKeyResult{
			Asm:       disbuf,
...
func handleGetWorkSubmission(s *rpcServer, hexData string) (interface{}, error) {
	...
	// blockchain.
	tempBlock := lddlutil.NewBlockDeepCopy(blockInfo.msgBlock)
...
func handleLiveTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	return lddljson.LiveTicketsResult{Tickets: ltString}, nil
}
...
func handleMissedTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	return lddljson.MissedTicketsResult{Tickets: mtString}, nil
}
...
// passed transaction.
func createVinListPrevOut(s *rpcServer, mtx *wire.MsgTx, chainParams *chaincfg.Params, vinExtra bool, filterAddrMap map[string]struct{}) ([]lddljson.VinPrevOut, error) {
	// Coinbase transactions only have a single txin by definition.
	if blockchain.IsCoinBaseTx(mtx) {
		...
		txIn := mtx.TxIn[0]
		vinList := make([]lddljson.VinPrevOut, 1)
		vinList[0].Coinbase = hex.EncodeToString(txIn.SignatureScript)
		amountIn := lddlutil.Amount(txIn.ValueIn).ToCoin()
	...
	vinList := make([]lddljson.VinPrevOut, 0, len(mtx.TxIn))
	...
	for i, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &lddljson.PrevOut{
				Addresses: encodedAddrs,
				Value:     lddlutil.Amount(originTxOut.Value).ToCoin(),
			}
		}
	}
...
func fetchMempoolTxnsForAddress(s *rpcServer, addr lddlutil.Address, numToSkip, numRequested uint32) ([]*lddlutil.Tx, uint32) {
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*lddljson.SearchRawTransactionsCmd)
	vinExtra := false
	if c.VinExtra != nil {
	...
	srtList := make([]lddljson.SearchRawTransactionsResult, len(addressTxns))
	for i := range addressTxns {
...
func handleSendRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*lddljson.SendRawTransactionCmd)
...
func min(s []lddlutil.Amount) lddlutil.Amount {
	if len(s) == 0 {
		return 0
	}
...
func max(s []lddlutil.Amount) lddlutil.Amount {
	max := lddlutil.Amount(0)
	for i := range s {
		if s[i] > max {
			max = s[i]
		}
...
func mean(s []lddlutil.Amount) lddlutil.Amount {
	sum := lddlutil.Amount(0)
	for i := range s {
		sum += s[i]
	}
	...
	return sum / lddlutil.Amount(len(s))
}

// median gets the median amount from a slice of amounts.
func median(s []lddlutil.Amount) lddlutil.Amount {
	if len(s) == 0 {
		return 0
	}
...
func stdDev(s []lddlutil.Amount) lddlutil.Amount {
	var total float64
	mean := mean(s)
	...
	amt, _ := lddlutil.NewAmount(math.Sqrt(v))
...
// memory pool.
func feeInfoForMempool(s *rpcServer, txType stake.TxType) *lddljson.FeeInfoMempool {
	txDs := s.server.txMemPool.TxDescs()
	ticketFees := make([]lddlutil.Amount, 0, len(txDs))
	for _, txD := range txDs {
		if txD.Type == txType {
			feePerKb := (lddlutil.Amount(txD.Fee)) * 1000 /
				lddlutil.Amount(txD.Tx.MsgTx().SerializeSize())
			ticketFees = append(ticketFees, feePerKb)
		}
	}

	return &lddljson.FeeInfoMempool{
...
// properly set.
func calcFeePerKb(tx *lddlutil.Tx) lddlutil.Amount {
	var in lddlutil.Amount
	for _, txIn := range tx.MsgTx().TxIn {
		in += lddlutil.Amount(txIn.ValueIn)
	}
	var out lddlutil.Amount
	for _, txOut := range tx.MsgTx().TxOut {
		out += lddlutil.Amount(txOut.Value)
	}

	return ((in - out) * 1000) / lddlutil.Amount(tx.MsgTx().SerializeSize())
}

// feeInfoForBlock fetches the ticket fee information for a given tx type in a
// block.
func ticketFeeInfoForBlock(s *rpcServer, height int64, txType stake.TxType) (*lddljson.FeeInfoBlock, error) {
	...
	txFees := make([]lddlutil.Amount, txNum)
	itr := 0
...
// from [start, end).
func ticketFeeInfoForRange(s *rpcServer, start int64, end int64, txType stake.TxType) (*lddljson.FeeInfoWindow, error) {
	hashes, err := s.chain.HeightRange(start, end)
	if err != nil {
		return nil, err
	}

	var txFees []lddlutil.Amount
	for i := range hashes {
...
func handleTicketFeeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*lddljson.TicketFeeInfoCmd)
	...
	var feeInfoBlocks []lddljson.FeeInfoBlock
	blocks := uint32(0)
	if c.Blocks != nil {
		blocks = *c.Blocks
	}
	...
	var feeInfoWindows []lddljson.FeeInfoWindow
	windows := uint32(0)
	if c.Windows != nil {
		windows = *c.Windows
	}
	...
	return &lddljson.TicketFeeInfoResult{
...
func handleTicketsForAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*lddljson.TicketsForAddressCmd)
	...
	reply := &lddljson.TicketsForAddressResult{
...
func handleTicketVWAP(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	return lddlutil.Amount(vwap).ToCoin(), nil
}
...
func handleTxFeeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	var feeInfoRange lddljson.FeeInfoRange
	...
	feeInfoRange = lddljson.FeeInfoRange{
	...
	return &lddljson.TxFeeInfoResult{
		FeeInfoMempool: *feeInfoMempool,
		FeeInfoBlocks:  feeInfoBlocks,
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*lddljson.ValidateAddressCmd)
	result := lddljson.ValidateAddressChainResult{}
	addr, err := lddlutil.DecodeAddress(c.Address)
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*lddljson.VerifyMessageCmd)
	...
	if wasCompressed {
		serializedPK = lddlPK.SerializeCompressed()
	} else {
		serializedPK = lddlPK.SerializeUncompressed()
	}
	address, err := lddlutil.NewAddressSecpPubKey(serializedPK,
		activeNetParams.Params)
	if err != nil {
...
func handleVersion(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	result := map[string]lddljson.VersionResult{
		"lddldjsonrpcapi": {
...
func (s *rpcServer) standardCmdResult(cmd *parsedRPCCmd, closeChan <-chan struct{}) (interface{}, error) {
	...
	return nil, lddljson.ErrRPCMethodNotFound
handled:
...
func parseCmd(request *lddljson.Request) *parsedRPCCmd {
	...
	if err != nil {
		...
		if jerr, ok := err.(lddljson.Error); ok &&
			jerr.Code == lddljson.ErrUnregisteredMethod {
			parsedCmd.err = lddljson.ErrRPCMethodNotFound
			return &parsedCmd
		}
...
// type *lddljson.RPCError to the appropriate type as needed.
func createMarshalledReply(rpcVersion string, id interface{}, result interface{}, replyErr error) ([]byte, error) {
	...
	if replyErr != nil {
		if jErr, ok := replyErr.(*lddljson.RPCError); ok {
			jsonErr = jErr
		} else {
	...
	return lddljson.MarshalResponse(rpcVersion, id, result, jsonErr)
}

// processRequest determines the incoming request type (single or batched),
// parses it and returns a marshalled response.
func (s *rpcServer) processRequest(request *lddljson.Request, isAdmin bool, closeChan <-chan struct{}) []byte {
...
func (s *rpcServer) jsonRPCRead(w http.ResponseWriter, r *http.Request, isAdmin bool) {
	...
	if !batchedRequest {
		...
		if err != nil {
			...
			resp, err = lddljson.MarshalResponse("1.0", nil, nil, jsonErr)
			if err != nil {
	...
	if batchedRequest {
		...
		if err != nil {
			...
			resp, err = lddljson.MarshalResponse("2.0", nil, nil, jsonErr)
			if err != nil {
				rpcsLog.Errorf("Failed to create reply: %v", err)
			}
			...
			if len(batchedRequests) > 0 {
				...
				for _, entry := range batchedRequests {
					...
					if err != nil {
						...
						resp, err = lddljson.MarshalResponse("", nil, nil, jsonErr)
						if err != nil {
							rpcsLog.Errorf("Failed to create reply: %v", err)
						}
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="lddld RPC"`)
