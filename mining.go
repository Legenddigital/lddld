...
func containsTxIns(txs []*lddlutil.Tx, tx *lddlutil.Tx) bool {
...
// can not be found.
func txIndexFromTxList(hash chainhash.Hash, list []*lddlutil.Tx) int {
	for i, tx := range list {
		h := tx.Hash()
		if hash == *h {
			return i
...
func UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int64, extraNonce uint64) error {
	...
	// TODO(davec): A lddlutil.Block should use saved in the state to avoid
	// recalculating all of the other transaction hashes.
	// block.Transactions[0].InvalidateCache()

	// Recalculate the merkle root with the updated extra nonce.
	block := lddlutil.NewBlockDeepCopyCoinbase(msgBlock)
...
func createCoinbaseTx(subsidyCache *blockchain.SubsidyCache, coinbaseScript []byte, opReturnPkScript []byte, nextBlockHeight int64, addr lddlutil.Address, voters uint16, params *chaincfg.Params) (*lddlutil.Tx, error) {
	tx := wire.NewMsgTx()
	...
	if nextBlockHeight == 1 && len(params.BlockOneLedger) != 0 {
		...
		addrs := make([]lddlutil.Address, len(params.BlockOneLedger))
		for i, payout := range params.BlockOneLedger {
			addr, err := lddlutil.DecodeAddress(payout.Address)
			if err != nil {
				return nil, err
			}
			addrs[i] = addr
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *lddlutil.Tx, height int64) error {
	for _, txIn := range tx.MsgTx().TxIn {
...
func logSkippedDeps(tx *lddlutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
func maybeInsertStakeTx(bm *blockManager, stx *lddlutil.Tx, treeValid bool) bool {
	missingInput := false
...
func deepCopyBlockTemplate(blockTemplate *BlockTemplate) *BlockTemplate {
	...
	coinbaseCopy :=
		lddlutil.NewTxDeep(blockTemplate.Block.Transactions[0])
...
func handleTooFewVoters(subsidyCache *blockchain.SubsidyCache, nextHeight int64, miningAddress lddlutil.Address, bm *blockManager) (*BlockTemplate, error) {
	...
	if nextHeight >= stakeValidationHeight {
		if bm.AggressiveMining {
			if curTemplate != nil {
				...
				block := lddlutil.NewBlockDeepCopyCoinbase(cptCopy.Block)
			...
			if curTemplate == nil {
				...
				btBlockTemp := lddlutil.NewBlockDeepCopyCoinbase(btMsgBlock)
				merkles :=
				...
				// Make sure the block validates.
				btBlock := lddlutil.NewBlockDeepCopyCoinbase(btMsgBlock)
...
func NewBlockTemplate(policy *mining.Policy, server *server, payToAddress lddlutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*lddlutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		// Consider caching this in the mempool in the future. - Legenddigital
	...
	blockTxnsStake := make([]*lddlutil.Tx, 0, len(blockTxns))
	...
	for _, tx := range blockTxns {
		...
		if stake.IsSSGen(msgTx) {
			txCopy := lddlutil.NewTxDeepTxIns(msgTx)
	...
	if nextBlockHeight < stakeValidationHeight {
		...
		for _, vb := range voteBitsVoters {
			if lddlutil.IsFlagSet16(vb, lddlutil.BlockValid) {
				voteYea++
			}
			totalVotes++
		}
		...
		if votebits == uint16(0x0000) {
			...
			// the previous block.
			// Legenddigital WARNING: The ideal behaviour should also be that we re-add
			...
			tempBlockTxns := make([]*lddlutil.Tx, 0, len(sourceTxns))
			for _, tx := range blockTxns {
				if tx.Tree() == wire.TxTreeRegular {
					...
					if isValid {
						txCopy := lddlutil.NewTxDeepTxIns(tx.MsgTx())
	...
	// Legenddigital: We need to move this downwards because of the requirements
	...
	blockTxnsRegular := make([]*lddlutil.Tx, 0, len(blockTxns)+1)
	...
	minrLog.Debugf("Created new block template (%d transactions, %d "+
		...
		lddlutil.Amount(msgBlock.Header.SBits).ToCoin())
