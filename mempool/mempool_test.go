...
type fakeChain struct {
	...
	blocks        map[chainhash.Hash]*lddlutil.Block
...
func (s *fakeChain) FetchUtxoView(tx *lddlutil.Tx, treeValid bool) (*blockchain.UtxoViewpoint, error) {
	s.RLock()
	defer s.RUnlock()
...
func (s *fakeChain) BlockByHash(hash *chainhash.Hash) (*lddlutil.Block, error) {
	s.RLock()
...
func (s *fakeChain) AddBlock(block *lddlutil.Block) {
	s.Lock()
...
func (s *fakeChain) CalcSequenceLock(tx *lddlutil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {
...
// transactions.
func txOutToSpendableOut(tx *lddlutil.Tx, outputNum uint32, tree int8) spendableOutput {
	return spendableOutput{
		outPoint: wire.OutPoint{Hash: *tx.Hash(), Index: outputNum, Tree: tree},
		amount:   lddlutil.Amount(tx.MsgTx().TxOut[outputNum].Value),
	}
}
...
// passed in.
func (p *poolHarness) GetScript(addr lddlutil.Address) ([]byte, error) {
...
func (p *poolHarness) GetKey(addr lddlutil.Address) (chainec.PrivateKey, bool, error) {
	return p.signKey, true, nil
}

// AddFakeUTXO creates a fake mined uxto for the provided transaction.
func (p *poolHarness) AddFakeUTXO(tx *lddlutil.Tx, blockHeight int64) {
...
// version 2 blocks.
func (p *poolHarness) CreateCoinbaseTx(blockHeight int64, numOutputs uint32) (*lddlutil.Tx, error) {
...
func (p *poolHarness) CreateSignedTx(inputs []spendableOutput, numOutputs uint32, expiry uint32) (*lddlutil.Tx, error) {
	// Calculate the total input amount and split it amongst the requested
	// number of outputs.
	var totalInput lddlutil.Amount
...
func (p *poolHarness) CreateTxChain(firstOutput spendableOutput, numTxns uint32) ([]*lddlutil.Tx, error) {
	txChain := make([]*lddlutil.Tx, 0, numTxns)
	...
	for i := uint32(0); i < numTxns; i++ {
		...
		txChain = append(txChain, lddlutil.NewTx(tx))
...
// output.
func (p *poolHarness) CreateTx(out spendableOutput) (*lddlutil.Tx, error) {
...
func (p *poolHarness) CreateTicketPurchase(sourceTx *lddlutil.Tx, cost int64) (*lddlutil.Tx, error) {
	ticketfee := lddlutil.Amount(singleInputTicketSize)
	ticketPrice := lddlutil.Amount(cost)
	...
	change := lddlutil.Amount(sourceTx.MsgTx().TxOut[0].Value) -
...
func (p *poolHarness) CreateVote(ticket *lddlutil.Tx) (*lddlutil.Tx, error) {
	...
	signedScript, err := txscript.SignTxOutput(p.chainParams, vote, inputToSign,
		...
		p, vote.TxIn[inputToSign].SignatureScript, lddlec.STEcdsaSecp256k1)
	if err != nil {
		return nil, err
	}
	...
	return lddlutil.NewTx(vote), nil
}

// CreateRevocation creates a revocation using the provided ticket.
func (p *poolHarness) CreateRevocation(ticket *lddlutil.Tx) (*lddlutil.Tx, error) {
	...
	signedScript, err := txscript.SignTxOutput(p.chainParams, revocation, inputToSign,
		redeemTicketScript, txscript.SigHashAll, p,
		p, revocation.TxIn[inputToSign].SignatureScript, lddlec.STEcdsaSecp256k1)
	if err != nil {
		return nil, err
	}
	...
	return lddlutil.NewTx(revocation), nil
}
...
func newPoolHarness(chainParams *chaincfg.Params) (*poolHarness, []spendableOutput, error) {
	...
	payPubKeyAddr, err := lddlutil.NewAddressSecpPubKey(pubKeyBytes,
		chainParams)
	if err != nil {
		return nil, nil, err
	}
	...
	chain := &fakeChain{
		...
		blocks:      make(map[chainhash.Hash]*lddlutil.Block),
...
func testPoolMembership(tc *testContext, tx *lddlutil.Tx, inOrphanPool, inTxPool bool) {
	txHash := tx.Hash()
...
func TestExpirationPruning(t *testing.T) {
	...
	expiringTxns := make([]*lddlutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
...
func TestBasicOrphanRemoval(t *testing.T) {
	...
	nonChainedOrphanTx, err := harness.CreateSignedTx([]spendableOutput{{
		amount:   lddlutil.Amount(5000000000),
