...
import (
	...
	"github.com/Legenddigital/lddld/blockchain/indexers"
	...
	"github.com/Legenddigital/lddld/mining"
...
const (
	...
	MinHighPriority = lddlutil.AtomsPerCoin * 144.0 / 250
	...
	// LDDL/kB, this results in a maximum allowed high fee of 1 LDDL/kB.
...
type Config struct {
	...
	FetchUtxoView func(*lddlutil.Tx, bool) (*blockchain.UtxoViewpoint, error)

	// BlockByHash defines the function use to fetch the block identified
	// by the given hash.
	BlockByHash func(*chainhash.Hash) (*lddlutil.Block, error)
	...
	// utxo view.
	CalcSequenceLock func(*lddlutil.Tx, *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error)
...
type TxPool struct {
	...
	orphans       map[chainhash.Hash]*lddlutil.Tx
	orphansByPrev map[wire.OutPoint]map[chainhash.Hash]*lddlutil.Tx
	outpoints     map[wire.OutPoint]*lddlutil.Tx
...
func (mp *TxPool) insertVote(ssgen *lddlutil.Tx) error {
	msgTx := ssgen.MsgTx()
	...
	vote := lddlutil.IsFlagSet16(voteBits, lddlutil.BlockValid)
...
// TODO Pruning of the votes map Legenddigital
...
func (mp *TxPool) removeOrphan(tx *lddlutil.Tx, removeRedeemers bool) {
	txHash := tx.Hash()
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveOrphan(tx *lddlutil.Tx) {
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addOrphan(tx *lddlutil.Tx) {
	...
	for _, txIn := range tx.MsgTx().TxIn {
		if _, exists := mp.orphansByPrev[txIn.PreviousOutPoint]; !exists {
			mp.orphansByPrev[txIn.PreviousOutPoint] =
				make(map[chainhash.Hash]*lddlutil.Tx)
		}
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAddOrphan(tx *lddlutil.Tx) error {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeOrphanDoubleSpends(tx *lddlutil.Tx) {
	msgTx := tx.MsgTx()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeTransaction(tx *lddlutil.Tx, removeRedeemers bool) {
	txHash := tx.Hash()
	if removeRedeemers {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveDoubleSpends(tx *lddlutil.Tx) {
	// Protect concurrent access.
	mp.mtx.Lock()
	for _, txIn := range tx.MsgTx().TxIn {
...
func (mp *TxPool) addTransaction(utxoView *blockchain.UtxoViewpoint,
	tx *lddlutil.Tx, txType stake.TxType, height int64, fee int64) {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) checkPoolDoubleSpend(tx *lddlutil.Tx, txType stake.TxType) error {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) fetchInputUtxos(tx *lddlutil.Tx) (*blockchain.UtxoViewpoint, error) {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) FetchTransaction(txHash *chainhash.Hash, includeRecentBlock bool) (*lddlutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.RLock()
	...
	// For Legenddigital, the latest block is considered "unconfirmed"
...
// It should also set the lddlutil tree type for the tx as well.
func (mp *TxPool) maybeAcceptTransaction(tx *lddlutil.Tx, isNew, rateLimit, allowHighFees, rejectDupOrphans bool) ([]*chainhash.Hash, error) {
	msgTx := tx.MsgTx()
	txHash := tx.Hash()
...
//
// This function is safe for concurrent access.
func (mp *TxPool) MaybeAcceptTransaction(tx *lddlutil.Tx, isNew, rateLimit bool) ([]*chainhash.Hash, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) processOrphans(acceptedTx *lddlutil.Tx) []*lddlutil.Tx {
	var acceptedTxns []*lddlutil.Tx
	...
	for processList.Len() > 0 {
		...
		processItem := firstElement.(*lddlutil.Tx)
...
//
// This function is safe for concurrent access.
func (mp *TxPool) ProcessTransaction(tx *lddlutil.Tx, allowOrphan, rateLimit, allowHighFees bool) ([]*lddlutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
	defer mp.mtx.Unlock()
	var err error
	...
	if len(missingParents) == 0 {
		...
		acceptedTxs := make([]*lddlutil.Tx, len(newTxs)+1)
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RawMempoolVerbose(filterType *stake.TxType) map[string]*lddljson.GetRawMempoolVerboseResult {
	mp.mtx.RLock()
	defer mp.mtx.RUnlock()

	result := make(map[string]*lddljson.GetRawMempoolVerboseResult,
		len(mp.pool))
	bestHeight := mp.cfg.BestHeight()
	...
	for _, desc := range mp.pool {
		...
		mpd := &lddljson.GetRawMempoolVerboseResult{
			Size:             int32(tx.MsgTx().SerializeSize()),
			Fee:              lddlutil.Amount(desc.Fee).ToCoin(),
...
func New(cfg *Config) *TxPool {
	return &TxPool{
		...
		orphans:       make(map[chainhash.Hash]*lddlutil.Tx),
		orphansByPrev: make(map[wire.OutPoint]map[chainhash.Hash]*lddlutil.Tx),
		outpoints:     make(map[wire.OutPoint]*lddlutil.Tx),
